...
- Definition of Done quality checks follow the PR template quality gate checklist (`.github/PULL_REQUEST_TEMPLATE.md`): run `mise run check`;
  when formatter/linter stability or WSL parity matters, also run `mise run check:wsl`;
...
| --- | --- | --- |
| Agent runtime entry, workflow, source routing, and this SoTT | [AGENTS.md](AGENTS.md) | Repository-local workflow, quality-gate selection, rule routing, and source ownership |
...
| SQLite schema and migrations | [src-tauri/src/infra/db/migration/](src-tauri/src/infra/db/migration/) and [src-tauri/src/infra/db/](src-tauri/src/infra/db/) | Embedded DB schema, migration ordering, connection policy, and SQLite repository implementations |
...
- If the durable answer is a rule, update `AGENTS.md` or `.claude/rules/` instead of keeping it as a task.
