...
export function useScreenSnapshot<T>(candidate: T | null, canAdopt: boolean): ScreenSnapshotResult<T> {
  ...
  // react-doctor-disable-next-line react-doctor/no-derived-state-effect -- false positive (snapshot latch, not derivable), docs/react-doctor-warning-classification-300.md:173
  useEffect(() => {
    if (!canAdopt || candidate === null) {
      return;
    }

    // react-doctor-disable-next-line react-doctor/no-derived-state -- false positive (snapshot latch, not derivable), docs/react-doctor-warning-classification-300.md:173
