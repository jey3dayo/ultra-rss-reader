...
describe("command args validation parity", () => {
  it("keeps frontend command name lengths aligned with Rust validation", () => {
    expect(renameAccountArgs.parse({ accountId: "acc-1", name: ` ${"a".repeat(ACCOUNT_NAME_MAX_CHARS)} ` })).toEqual({
      accountId: "acc-1",
      name: "a".repeat(ACCOUNT_NAME_MAX_CHARS),
    });
    expect(renameFeedArgs.parse({ feedId: "feed-1", title: ` ${"a".repeat(FEED_TITLE_MAX_CHARS)} ` })).toEqual({
      feedId: "feed-1",
      title: "a".repeat(FEED_TITLE_MAX_CHARS),
    });
    expect(createFolderArgs.parse({ accountId: "acc-1", name: ` ${"a".repeat(FOLDER_NAME_MAX_CHARS)} ` })).toEqual({
      accountId: "acc-1",
      name: "a".repeat(FOLDER_NAME_MAX_CHARS),
    });
    expect(renameTagArgs.parse({ tagId: "tag-1", name: ` ${"a".repeat(TAG_NAME_MAX_CHARS)} ` })).toEqual({
      tagId: "tag-1",
      name: "a".repeat(TAG_NAME_MAX_CHARS),
    });

    expect(() => renameAccountArgs.parse({ accountId: "acc-1", name: "a".repeat(ACCOUNT_NAME_MAX_CHARS + 1) })).toThrow(
      `Account name must be ${ACCOUNT_NAME_MAX_CHARS} characters or less`,
    );
    expect(() => renameFeedArgs.parse({ feedId: "feed-1", title: "a".repeat(FEED_TITLE_MAX_CHARS + 1) })).toThrow(
      `Feed title must be ${FEED_TITLE_MAX_CHARS} characters or less`,
    );
    expect(() => createFolderArgs.parse({ accountId: "acc-1", name: "a".repeat(FOLDER_NAME_MAX_CHARS + 1) })).toThrow(
      `Folder name must be ${FOLDER_NAME_MAX_CHARS} characters or less`,
    );
    expect(() => renameTagArgs.parse({ tagId: "tag-1", name: "a".repeat(TAG_NAME_MAX_CHARS + 1) })).toThrow(
    ...
    expect(extractRustUsizeConst(readRustCommandSource("feed_commands.rs"), "FOLDER_NAME_MAX_CHARS")).toBe(
      FOLDER_NAME_MAX_CHARS,
    );
