...
describe("article-browser-actions", () => {
  ...
  it("routes reading-list command diagnostics through URL-token redaction", async () => {
    ...
    const addToReadingListSpy = vi
      .spyOn(tauriCommands, "addToReadingList")
      .mockRejectedValue(new Error("osascript failed for https://user:pass@example.com/private?token=raw#frag"));
    ...
    expect(serialized).toContain("https://example.com/private?redacted#redacted");
    expect(serialized).not.toContain("user:pass");
    expect(serialized).not.toContain("token=raw");
    expect(serialized).not.toContain("#frag");
    expect(showToast).toHaveBeenCalledWith("osascript failed for https://user:pass@example.com/private?token=raw#frag");
    addToReadingListSpy.mockRestore();
  });
