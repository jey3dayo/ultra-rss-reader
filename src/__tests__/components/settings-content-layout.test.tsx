...
describe("SettingsContentLayout", () => {
  it("uses shell content and field-label tones in stacked-left mode", () => {
    render(
      <SettingsContentLayout
        title="General"
        subtitle="Tune reading behavior."
    ...
    expect(screen.getByTestId("settings-content-header")).toHaveClass(
      "min-h-[4.5rem]",
      "py-0",
      "items-center",
    );
    ...
    expect(screen.getByText("Tune reading behavior.")).toHaveClass(
      "text-[color:var(--settings-shell-section-label)]",
    );
    expect(screen.getByText("Tune reading behavior.")).toHaveClass(
      "text-[13px]",
    );
  ...
  it("keeps overflow fades decorative and token-driven", () => {
    ...
    expect(screen.getByTestId("settings-content-fade-top")).toHaveAttribute(
      "aria-hidden",
      "true",
    );
    expect(screen.getByTestId("settings-content-fade-top")).toHaveClass(
      "pointer-events-none",
    );
    expect(screen.getByTestId("settings-content-fade-top")).toHaveStyle({
      backgroundImage: "var(--settings-shell-content-fade)",
    });
    expect(screen.getByTestId("settings-content-fade-bottom")).toHaveAttribute(
      "aria-hidden",
      "true",
    );
    expect(screen.getByTestId("settings-content-fade-bottom")).toHaveClass(
      "pointer-events-none",
    );
