  ...
  if (surface === "flat") {
    return (
      <section className={className}>
        <SectionHeading className={cn("mb-2 sm:mb-3", headingClassName)}>{heading}</SectionHeading>
        <div className={contentClassName}>{children}</div>
        {note ? <p className="mt-1 font-serif text-xs leading-[1.45] text-foreground-soft sm:mt-1.5">{note}</p> : null}
      </section>
    );
  }
