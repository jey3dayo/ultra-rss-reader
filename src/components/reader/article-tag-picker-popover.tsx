...
export function ArticleTagPickerPopover({
  ...
  return (
    <div
      ...
      className="absolute top-full left-0 z-50 mt-2 min-w-[220px] rounded-xl border border-border/80 bg-popover/98 p-1.5 shadow-lg"
    ...
    >
      {availableTags.map((tag, index) => (
        <button
          ...
          className="flex min-h-11 w-full items-center gap-2 rounded-lg px-3 py-2.5 text-left text-sm text-popover-foreground hover:bg-accent/80"
        >
          {tag.color && (
      ...
      <div className="mt-1 flex items-center gap-1.5 border-t border-border/80 px-2 pt-2">
        ...
        <Button
          ...
          className="h-10 w-10 rounded-lg text-muted-foreground hover:bg-accent/80"
