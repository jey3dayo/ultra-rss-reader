...
export function useSidebarFeedDragState({
  ...
  const clearDragState = useCallback(() => {
    // react-doctor-disable-next-line react-doctor/no-adjust-state-on-prop-change -- accepted risk (drag session teardown), docs/react-doctor-warning-classification-249.md
  ...
  useEffect(() => {
    ...
    if (activeDropTarget.kind === "folder") {
      if (!canDropFeedToFolder(draggedFeedId, activeDropTarget.folderId)) {
        // react-doctor-disable-next-line react-doctor/no-adjust-state-on-prop-change -- accepted risk (drag hover invalidation), docs/react-doctor-warning-classification-249.md
        setActiveDropTarget(null);
      }
      return;
    }
