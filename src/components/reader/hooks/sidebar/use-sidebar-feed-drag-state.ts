...
export function useSidebarFeedDragState({
  ...
  useEffect(() => {
    ...
    if (activeDropTarget.kind === "folder") {
      // react-doctor-disable-next-line react-doctor/no-pass-live-state-to-parent -- false positive (local useState setter, not a parent prop), docs/react-doctor-warning-classification-300.md:160
