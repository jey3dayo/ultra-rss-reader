...
function normalizeSidebarExpandedFoldersStorage(raw: string): SidebarExpandedFoldersStorage | null {
  ...
  if (!parsed) {
    logSidebarExpandedFoldersStorageFailure("parse-invalid", new Error("Invalid sidebar expanded folders storage"));
...
export function useSidebarStartupFolderExpansion({
  ...
  useEffect(() => {
    ...
    // react-doctor-disable-next-line react-doctor/no-pass-live-state-to-parent -- accepted risk (external store convergent correction), docs/react-doctor-warning-classification-300.md:219
