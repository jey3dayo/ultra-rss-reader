...
- 日本語入力の変換中に押した Enter や Escape で、アカウント名の編集が壊れる問題を修正した。変換確定のつもりの Enter で半端な名前が保存され、候補を取り消すつもりの Escape で編集そのものが破棄されていた。
- 画面の更新中に押した操作が、古い状態を見て取り消される場合があった問題を修正した。アカウントを切り替えた直後の名前変更や接続テスト、フィード追加中の URL 判定、タグの作成と名前変更が対象で、有効な操作結果が「古い応答」と誤判定されて捨てられることがあった。
- 記事本文中のリンクを開いたとき、直前まで表示していた記事を基準に URL が解決される場合があった問題を修正した。フィードのドラッグ操作と購読一覧のスクロール位置の通知にも同じ原因があった。
