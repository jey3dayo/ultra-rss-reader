...
- React Doctor の baseline が実態と食い違っていないかを CI で検証するようにした。件数の合計だけでなく、報告された診断の集合そのものを分類記録と照合する。合計が一致したまま中身が入れ替わる取りこぼしを防ぐ。(#324, #325, #326)
- 上記の検証で見つかった分類漏れを 1 件修正した。サイドバーのフィード行が未分類のまま計上されていた。(#325)
- 依存パッケージの更新で `mise.toml` と `package.json` の pnpm のバージョン指定がずれていたのを揃えた。
- 完了したタスクを整理した。
...
- プロンプト面の棚卸しで見つかった、現状と食い違う記述を削除した。(#322)

## [0.63.4] - 2026-09-18

### Bug Fixes

- サイドバーでタイトルが長いフィードとフォルダの行が、名前を省略せずに右へはみ出し、未読件数が画面外へ押し出されて見えなくなる問題を修正した。v0.63.2 で行の収縮アニメーションを入れたときに混入した回帰で、日本語のタイトルのように折り返し位置を持たない名前ほど起きやすかった。(#309)
- 購読一覧で、フィードの削除中にアカウントを切り替え、その削除が失敗した場合に、切り替え先のアカウントを表示しているのに前のアカウントのフィードの削除確認ダイアログが残り続ける問題を修正した。(#299)

### Maintenance

- 購読一覧ページを機能単位の hook へ分割した。(#317)
- React Doctor の第 2 波 21 件を分類し、baseline を再 pin した。(#311, #312, #313)
- 調査が完了または保留になったタスクを整理した。

## [0.63.3] - 2026-09-15

### Bug Fixes

- 同期中に操作したときのエラー通知が英語のまま表示される問題を修正した。フィードの購読解除・追加・編集、フォルダの作成・変更、閲覧履歴の消去、記事の既読・スター操作、リンクのコピーと外部ブラウザで開く操作、アカウント接続の各画面が対象。翻訳は用意されていたが、これらの画面から使われていなかった。
- 確認ダイアログで Enter を押すと、操作が実行されずに閉じるだけだった問題を修正した。購読解除やタグ削除を含め、すべての確認ダイアログで最初から実行ボタンにフォーカスが当たる。
- フィードの追加・編集、タグの作成・名前変更のダイアログで、Enter を押しても保存されない問題を修正した。フィード追加ダイアログでは入力欄が 2 つあるため常に効いていなかった。
- 設定のアカウント名編集で、日本語入力の変換候補を Enter で確定すると、その Enter がそのまま名前の保存として扱われる問題を修正した。macOS のアプリで起きていた。

### Documentation

- ダイアログの Enter 確定ルールを追加し、実装と食い違っていたルール・設計の記述を修正した。

### Maintenance

- rustls を 0.23.45 へ更新した。TLS 1.3 のハンドシェイクメッセージが暗号化レベルの境界を越えて受理される問題 (RUSTSEC-2026-0285) の修正を含む。
- ローカライズテストを代表 1 件に整理した。

## [0.63.2] - 2026-09-11

### Features

- サイドバーのフィード一覧で、未読が 0 になった行が即座に消えるのではなく、収縮アニメーションで閉じるようにした。(#302)

### Bug Fixes

- 日本語入力の変換中に押した Enter や Escape で、アカウント名の編集が壊れる問題を修正した。変換確定のつもりの Enter で半端な名前が保存され、候補を取り消すつもりの Escape で編集そのものが破棄されていた。(#293)
- 画面の更新中に押した操作が、古い状態を見て取り消される場合があった問題を修正した。アカウントを切り替えた直後の名前変更や接続テスト、フィード追加中の URL 判定、タグの作成と名前変更が対象で、有効な操作結果が「古い応答」と誤判定されて捨てられることがあった。(#292)
- 記事本文中のリンクを開いたとき、直前まで表示していた記事を基準に URL が解決される場合があった問題を修正した。フィードのドラッグ操作と購読一覧のスクロール位置の通知にも同じ原因があった。(#291)
- アカウントを切り替えたあとに、前のアカウントのフィード編集ダイアログが残る問題を修正した。(#301)
- 記事一覧の読み込みに失敗したとき、コマンド履歴が失われる問題を修正した。(#297)

### Maintenance

- 記事一覧と検索フォールバックの再描画コストを削減した。(#288, #298)
- motion 関連の CSS を `global.css` から切り出した。(#305)
- pnpm の `no-downgrade` trust policy を採用した。(#289)
- React Doctor の警告分類と baseline 更新、Knip / vitest baseline の実測合わせを行った。(#287, #294, #303, #304, #306, #307)

### Documentation

- React Doctor の finding 分類記録と、サイドバー退場アニメーションで踏んだ 2 つの罠をルール化した。(#286, #290, #296, #308)

## [0.63.1] - 2026-09-10

### Bug Fixes

- 日本語入力の変換中に押した Enter や Escape が、ショートカット操作として扱われる箇所を塞いだ。記事一覧にフォーカスがある状態での変換確定と、ショートカット設定でキーを記録している最中の変換キャンセルが対象。前者は既読切り替えなどが誤って走り、後者は設定画面が閉じて記録が中断されていた。

### Maintenance

- 依存関係の集計処理が、pnpm の新しい 2 文書形式のロックファイルを読んだときにパッケージマネージャ自身を依存として数えないようにした。現在のロックファイルは単一文書なので、集計結果は変わらない。
