...
- 確認ダイアログを作り直した。装飾アイコンと視覚的に隠したタイトルという構成をやめ、意味を持つアイコンバッジと可視の見出しを左揃えで並べ、説明文を 1 本にまとめた。取り消しと実行のボタンは右端に寄せ、幅が狭いときはラベルが折り返して縦に積み直る。
- 確認ダイアログの見出しが、汎用的な「確認」ではなく実行しようとしている操作名（アカウント削除なら「削除」）になった。読み上げ時のダイアログ名も同じ操作名になる。

### Bug Fixes

- 日本語入力の変換中に Enter や Escape を押したとき、タグが作成されたり検索が閉じたりしてしまう問題を修正した。変換の確定操作が誤って画面操作として扱われなくなる。
- アプリ再起動の確認ダイアログで、見出しと説明に同じ文言が並んでいたのを直した。説明に確認文を表示する。
- 確認ダイアログのボタンで、ラベルが長く折り返したときに文字が欠けたりボタンからはみ出したりする問題を修正した。

### Maintenance

- 開発用ツールチェーンを pnpm 12 と Vitest 5 へ更新した。アプリの動作は変わらない。

## [0.62.1] - 2026-09-07

### Maintenance

- 記事が自動で既読にならない事象を次回発生時に切り分けられるよう、ローカルログへ診断を記録するようにした。記録するのは処理の段階と所要時間と結果の分類のみで、記事やフィードの情報、URL、認証情報は含まない。既読の動作とタイミングは変えていない。
...
- 選択サマリーの指標を、サマリー面の幅が狭いときに 1 列へ縦積みするようにした。ウィンドウ幅ではなくサマリー面の幅で切り替わる。
- 設定プロファイルの取り込みを 1 MiB までに制限した。超過するファイルは読み込み前に拒否される。
- デスクトップ表示で、選択サマリーと記事一覧の空状態カードの上端をそろえた。カードの中身の量では位置が変わらず、画面が低くて収まらないときだけそのペインが上寄せになる。

### Fixed

- 記事一覧が最初の取得ぶんより先を読み込めなかった問題を修正した。
- 一覧がウィンドウに収まりきってスクロールできないとき、続きの記事が読み込まれないまま止まる問題を修正した。
- 記事が非常に多いフィードで、ページング位置が上限を超えた要求を出して読み込みが失敗する問題を修正した。
- 読み込み中に表示される仮の行が、読み込み後の記事行と横位置がずれていた問題を修正した。
- リーディングリストへの送信で、ローカルホストおよびプライベート IP のアドレスを拒否するようにした。
- 設定した時間が過ぎても記事が自動で既読にならないことがある問題を修正した。画面の再描画が続くと待機中のタイマーが取り消され続けていた。
...
- タグ付与チップとドラッグ＆ドロップ後のフィード行に登場アニメーションを追加した(#225)。

### Changed

- コマンドパレットを即時表示・開閉にし、記事スライドとリサイズ面のモーションを調整した(#215、#229)。

### Fixed

- アカウントをまたいだローカル取り込みで、購読やタグが混入する問題を修正した(#216)。
- エクスポート先ディレクトリのsymlinkによる書き込み先のすり替えを拒否するようにした(#226)。
- reduced-motion設定でアニメーションが残り、ローディング表示が止まって見える問題を修正した(#212、#229、#230)。
- 診断ログに生データが含まれる問題を修正した(#213)。
...
- ミュートによる自動既読化で、記事に対するスター操作の保留状態が消える問題を修正した(#198)。
- LAN・self-hosted 環境の Web Preview で、プライベート URL を開けない問題を修正した(#199)。
- GReader 同期のページネーションが継続トークンの循環や無制限の応答で占有される問題を修正した(#202)。
- reduced-motion 環境でスピナーが回り続ける問題を修正した(#204)。
- コマンドパレットで入力するたびに結果リストがちらつく問題を修正した(#203)。

## [0.60.0] - 2026-08-25

### Features

- 類似度レポートの対象にCSSを追加し、スタイルの重複も継続的に確認できるようにした。
- 設定プロフィールのエクスポートで、FreshRSSアカウントに関係する識別子の表示範囲を整理した。
- 利用できないプロバイダーをアカウント追加画面の案内対象から除外した。

### Bug Fixes

- 同期の再調整で、直前に行った既読操作が古い状態へ戻らないようにした。
- 記事の保留中操作と最近の閲覧履歴キャッシュを、遅れて届く更新や失敗時の復元を考慮して扱うようにした。
- 記事操作が対象行に一致しない場合にDBエラーとして扱い、状態不整合を見逃さないようにした。
- 設定画面の表示モード保存結果が古い場合に成功扱いしないようにした。
- 利用できないプロバイダーをアカウント追加画面に表示しないようにした。
- FreshRSSのURL検証を共通化し、IPv4-mapped IPv6を含む私設ホストの回避経路を防いだ(#168)。
- パッケージ版で開発用credential storeが有効にならないようにした。
- ローカルアカウントとFreshRSSアカウントのエクスポート識別子に関する説明範囲を正しくした。

### Documentation

- 設定プロフィールのキャンセル・エクスポート説明とプライバシー説明を、実装の契約に合わせた。
- 古いLinux/Feedlyロードマップと、実装済み設定プロフィールの記述を整理した。

### Maintenance

- Sentry、React Query、開発用依存関係を更新した。
- 設定画面、記事一覧、同期スケジューラの責務を分割し、公開契約を維持した(#166、#167)。
- CSS共有モーションセレクタ、類似度ベースライン、同期・リリース契約テストを整理・強化した。
- ジェネリック検査、i18nストーリー検査、重いリリースfixtureのテスト安定性を改善した。
- 完了した作業項目とリリース自動化関連の追跡状態を整理した(#189、#191)。

## [0.59.1] - 2026-08-24

### Bug Fixes

- リリース用の依存関係SBOMをmacOS・Windowsのターゲットごとに生成し、各プラットフォームのライセンス構成を正しく検証できるようにした(#162)。
- Cargo.lockのバージョン更新で書式を保持し、5つのバージョン所有箇所の更新結果を検証するようにした(#165)。

### Maintenance

- リリース時のバージョン更新を自動化し、`package.json`、Cargo.toml、Cargo.lock、Tauri設定、MSIXマニフェストの5箇所を一括で同期するようにした(#161)。
- フィード検出とローカルプロバイダーの内部実装を責務別に分割し、既存の公開契約を保ったまま保守性を高めた(#160、#163)。
- 類似型ペアの検出ベースラインを更新し、今後の重複コード検出を継続的に確認できるようにした(#164)。

## [0.59.0] - 2026-08-24

### Features

- 同期警告(再試行待ちの変更、スキップしたエントリ、フォルダ削除、ローカル同期の失敗など)の詳細が日本語で表示されるようになった。既読化・スター付与などの操作名や取り込み・書き出しの区分も翻訳される(#62、#90)。

### Bug Fixes

- 設定のアカウント詳細に表示される同期エラーを日本語化した(既知のプレフィックスは翻訳し、未知の値は原文を表示するフォールバックを維持)(#109)。
- 手動アカウント同期中も、サイドバーの更新ボタンが進行中として表示されるようにした(#115)。
- 同期完了後、フィード一覧の再取得が終わるまで同期ボタンの回転を継続するようにした(最新の未読数が表示されるまで状態を示す)(#117)。
- コマンド層の同期ログにアカウント名・フィード URL・フィードタイトルを記録せず、ID とホスト種別だけを記録するようにした(#153)。
- 複数アカウント運用時、アカウントを切り替えるとサイドバーのスター件数・タグ別件数・フィード別サマリが読み込み中のまま更新されない問題を修正した(切り替え時のクエリ取り消しが、切り替え先アカウントの取得まで打ち切っていた)。
- サイドバーのフィード別スター数が、スター記事が50件を超えると実際より少なく表示される問題を修正した(件数を集計クエリから取得するように変更)。
...
- 500行を超えていた Rust/TypeScript モジュールを責務別に分割し、テスト配置と共通 fixture を整理した(公開 API とテスト契約は維持)。
- 開発用 MCP ブリッジプラグインをビルド時オプション(非デフォルト feature)に分離し、リリースビルドの依存グラフに含まれないようにした(#87)。
...
- Web Preview内のページ遷移をhttp/httpsのみに制限し、`javascript:`・`file:`・`data:`・資格情報付きURLへのリダイレクトを遮断した。LAN上の自己ホストサイトのプレビューは従来どおり利用できる。
- FreshRSSのフィード名変更・フォルダ移動を同期境界のガード内で実行し、同期中の古い購読スナップショットによる巻き戻りを防いだ。
- 起動時のリモート既読状態修復を同期ガード取得後に実行し、手動・自動同期との並行実行による既読状態の競合を防いだ。
- ローカル同期フォルダの走査に深さ・シンボリックリンク・ファイルサイズの上限を設け、異常なディレクトリ構成でも同期処理が破綻しないようにした。スキップしたファイルは警告として記録される。
...
- CONTRIBUTINGのservice構成表を実在モジュールに合わせ、dotenvx管理の`.env`のコミット可否の記述を実態に修正し、lefthookフックのインストール手順を追加した。
...
- 更新通知の操作間隔と主従関係を整理し、本文と操作ラベルの左端を揃えた。
- フィード追加・編集画面の入力レールと操作ボタン寸法を共有コンポーネントで統一した。
- フィード編集画面の購読解除説明文が、十分な画面幅でも2行に折り返される問題を修正した。
...
- サイドバーの購読セクション・フォルダ・タグの開閉をアニメーション化した。閉じたパネルは DOM に残したまま操作とスクリーンリーダーから除外する。
- 記事リストの読み込み中表示を、テキストから記事行の形に沿ったスケルトンに置き換えた。

### Bug Fixes

- feed-rs を 2.4.0 に更新し、quick-xml の advisory（RUSTSEC-2026-0194 / 0195）に対応した。外部・入れ子エンティティを展開しない挙動は回帰テストで固定している。
- 購読詳細のリンクを「コンテンツの URL」「RSS フィード」の順に並べ替えた。
- 一括既読の確認ダイアログで、説明文とボタンの両方に件数が出る重複を解消した。
- アプリのファビコンが存在しないファイルを参照していた問題を修正し、テーマに追従するアイコンに差し替えた。

### Maintenance

- ダイアログとポップアップのモーションを、表示 200ms・非表示 120ms の非対称タイミングに分離した。`prefers-reduced-motion` では移動と拡大縮小を止めた上で短いクロスフェードを残す。
- 記事リストの未読ドットを、瞬時の表示切替からフェードに変更した。
- semantic state（warning / danger / success）の面色を単一ソース化し、hover 用の派生トークンを追加した。従来 hover 時に色が変化しなかったボタンが反応するようになった。
- 参照のないトークンとモジュール（`state-review-*`、`exception-palettes`）を削除した。
- pnpm を 11.21.0、lucide-react を 1.31.0、react-doctor を 0.9.11 に更新した。
...
- 記事メニューから対象フィードの編集画面を開き、ソースフィードを直接編集できるようにした。
- フィード編集画面から購読解除できるようにした。

### Bug Fixes

- ローカライズされたUI文言が表示領域に収まりやすいよう、フレーズ単位の折り返しを共通化した。
- FreshRSS同期や古い記事の削除処理で、フィードの最新活動情報が失われないようにした。
- CIのpnpm契約テストをpnpm 11.18.0に同期した。

### Documentation

- エンティティエディターの階層ルールをデザインガイドへ追加した。

### Maintenance

- pnpmと関連パッケージを更新した。
- 生成されたPlaywrightレポートをMarkdown lintの対象外にした。

## [0.54.6] - 2026-07-29

### Features

- フォルダ・タグ・スマートビューの概要画面に並ぶフィードカードでも、右クリックでフィードのコンテキストメニュー（編集、サイトを開く、すべて既読、古い未読を既読、購読解除）を開けるようにした。サイドバーのフィード行と同じ共有コンポーネントを使う。
...
- ウィンドウサイズを終了時に保存し、次回起動時に復元するようにした。復元位置は常に現在のディスプレイ中央で、外部モニタの取り外しやDPIの異なるディスプレイへ移った場合は現在のモニタに収まるサイズへ補正する。ウィンドウ位置と最大化・フルスクリーン状態は復元せず、最大化して終了した場合は最後の非最大化サイズで開く。

### Bug Fixes

- 同期メンテナンス中に操作したときのトーストが英語のまま表示されていたのを、表示言語に合わせて出すよう修正した。
- アプリ内ブラウザのテーマ切り替えアニメーションが長く感じられたため、500msへ短縮した。

### Maintenance

- Storybookを10.5.5へ、jsdomを30.0.0へ更新した。
- `TODO.md` の書式を triage ツールが解釈できる形へ統一し、書式契約を repo-local スキル `todo-triage-format` として記録した。
...
- コマンドパレットからフォルダやスマートビューを開けるようにし、フィルターやシステムテーマ切り替えのアクションも追加した。

### Bug Fixes

- フォルダサマリーがフィードリストを圧迫してクリップされていたのを、サマリー側をスクロール可能にして解消した。
...
- 選択サマリーペイン（フォルダ/フィード概要画面）を再設計。数字の視認性を高め、最終更新をヘッダーへ分離し、対象スコープの全フィードを未読数順に一覧表示するようにした。未読0件のフィードは減光し、フィードカードはホバー/押下フィードバック付きでクリック可能にした。
- 掲示板形式フィードの記事プレビューから、投稿ヘッダー・裸URL・レスID断片などのノイズを除去し、本来の要約テキストだけを表示するようにした。

### Bug Fixes

- 記事リストの未読/スターフィルターのアクティブ状態が、対応するセマンティックトーン（未読=青系、スター=アンバー系）で発色するよう修正した。ホバー/選択色をトーン側が一貫して持つようにした。
- サイドバーで非アクティブなペインのフィード/フォルダ選択が、ホバー状態と見分けがつかなかった問題を修正し、選択中の塗りを常に明確にした。

### Maintenance

- 記事リストのフッターを、既存のヘッダーと同じ半透明（translucent）クロムに統一した。
- 記事リスト・記事ペインの空状態（「記事がありません」「記事を選んでください」）を、枠付きカードから中央揃えの余白基調レイアウトへ刷新し、狭いペインでの圧迫感を解消した。
- 記事の既読/未読キャッシュ更新まわりの重複ヘルパーを整理し、`article-read-projection` ポリシーモジュールへ処理を委譲した。

## [0.53.42] - 2026-07-20

### Bug Fixes

- フォルダ/フィードの一括既読化後にサイドバーの未読件数だけが減り、記事リストの未読ドットや記事詳細の既読ボタンが更新されない同期ずれを修正した。バックエンドが既読化した記事IDを返すようになり、フロントは記事リスト・記事詳細キャッシュを即時パッチする。
- 未読表示でフォルダ/フィードを一括既読化した際、単一記事の既読化と同様に行をリストに残したまま未読ドットだけを消すようにした（画面を切り替えるまで表示を保持）。

### Documentation

- 一括既読同期修正の記録と、retention 上限（50件）と一括既読の仕様判断 TODO を追加した。
...
- サイドバーのスマートビュー（未読/スター）を選択した際の背景トーンが淡すぎて選択状態が分かりにくかったため、コントラストを強めて選択中であることが視認できるようにした。
- 購読一覧のフォルダツリーの罫線表示を廃止してシンプルなインデント表示にし、詳細ペインの操作ボタンをスクロール追従ではなく固定フッターにするなど、デザインレビューに基づき購読一覧・詳細画面を調整した。
- 購読レビューの再確認理由に表示される未更新日数が固定のしきい値表示になっていた問題を修正し、実際の未更新日数を表示するようにした。
...
- 記事末尾で「次の記事」ボタンが押せるのに何も起きない問題を修正し、最後の記事ではボタンを非表示にした。
- ローカル同期の自動同期トグルに、フォルダ保存とは別の専用トースト文言を追加した。
...
- 購読詳細に「更新頻度」を表示し、更新が多すぎる購読だけに絞り込める「更新過多」フィルタを購読一覧に追加した。

### Bug Fixes

- フィードツリーでフォルダの選択と展開トグルを分離し、選択済みフォルダの再クリックで意図せず折りたたまれないようにした。
- 購読一覧のフォルダ行の見た目を整理した。
- 購読詳細パネルの見直し理由と操作エリアの枠線を調整した。
- 購読詳細のリンクにフィード/コンテンツのアイコンを追加した。
- 購読一覧のワークスペース枠を整え、フィードリンクを表示できるようにした。
- React 19 の innerHTML リセットによる画像のちらつきを修正した。
...
- 購読一覧の未更新表示を「90日以上未更新」「長期未更新」に整理し、状態を事実ベースで確認しやすくした。
- 購読一覧のワークスペース枠を軽くし、サマリー切り替え時の表示を滑らかにした。
...
- 購読の見直し候補を「30日以上更新なし」「60日以上更新なし・未読0件」「90日以上更新なし」の段階表示に整理し、詳細画面に判定理由と更新なし日数を表示するよう修正した。
- 購読一覧の検索・管理操作を整理し、重複していた検索クリア操作をなくした。
...
- 記事一覧ツールバーの無効化ボタン(例: すべて既読)で、有効時と見分けにくかった disabled 表現を修正した。
- 記事一覧を検索して結果をクリックした際に、稀に「記事が見つからない」と表示される問題を修正した。記事本体の取得を単一の取得元に統一した。

### Changed

- 通知トーストの閉じる/アクションボタンをゴースト表現に統一し、ホバー時に背景が塗られないようにした。

## [0.53.22] - 2026-07-08

### Changed

- アプリ内アップデートを、起動時チェックで検出した更新を完全サイレントで裏ダウンロードし、準備完了後に一度だけ「再起動後に適用されます」トーストで知らせる方式に変更した。「今すぐ再起動」は確認ダイアログなしで即座に再起動する。コマンドパレットからの手動チェックのみ、これまで通りダウンロード進捗トーストを表示する。ダウンロードは同期中でも開始でき、同期と競合した場合はインストールだけを再起動時まで遅延する。
...
- フォルダの選択サマリー画面にある「最近のフィード」カードからフィードをクリックして選択できるようにした。選択したフィードはサイドバーの購読ツリー内でも自動的にスクロール表示され、ハイライトされる。
- マウスの戻るボタン(サイドボタン)で、選択中のフィードからそのフィードが属するフォルダへ戻れるようにした。記事表示中はまず記事を閉じてフィード/フォルダのサマリー画面に戻り、続けて戻るボタンを押すとフォルダの選択画面まで戻る。

## [0.53.19] - 2026-07-05

### Bug Fixes

- Webプレビュー表示中、閉じる/戻る/すすむ/リロードボタンのツールチップがネイティブWebViewの裏に隠れて見えなくなる問題を修正した。右側のツールボタン群と同じ横開きのツールチップに統一した。
- Webプレビューのツールバーで、無効化された(disabled)ボタンにマウスを乗せると、実際は操作できないのにホバー色が変化して押せるように見えてしまう問題を修正した。

## [0.53.18] - 2026-07-05

### Bug Fixes

- フィードやフォルダを選択したときに、設定「フィード/フォルダ選択時に最初の記事を開く」が OFF でも先頭記事が自動で開いてしまう問題を修正した。OFF(デフォルト)ではフォルダ/フィードの選択サマリー画面を表示し、先頭記事を自動で開かないようにした。スマートビュー(未読/スター/最近の記事)の挙動は従来どおり。設定ラベルもフォルダ選択に対応する文言へ更新した。

## [0.53.17] - 2026-07-05

### Bug Fixes

- OPML エクスポートのネイティブ保存ダイアログで、拡張子なしのファイル名(例: `feeds`)を指定した際に自動付与される `.opml` パスへ確認なしで上書きしてしまう問題を修正した。付与後のパスに既存ファイルがある場合はエラーとして拒否し、ユーザーが明示的に `.opml` を入力した場合のみ上書きを許可する。
