...
- 購読解除やタグ削除の確認ダイアログで、Enter を押しても実行されない問題を修正した。
...
- 確認ダイアログで Enter を押すと、操作が実行されずに閉じるだけだった問題を修正した。「既読にする」などの確認では最初から実行ボタンにフォーカスが当たるようにした。購読解除のような取り消せない操作は、誤操作を防ぐため従来どおりキャンセル側にフォーカスが残る。
