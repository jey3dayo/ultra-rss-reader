...
- フィード取得と FreshRSS 通信の HTTP クライアントが、初期化に失敗すると防御設定なしの既定クライアントで黙って通信を続ける問題を修正した。あわせて FreshRSS 通信のリダイレクトにもフィード取得と同じ防御(回数上限・HTTPS から HTTP への格下げ拒否・私設アドレスの再検証)を適用した。
- FreshRSS の未読同期で、未読一覧の取得が途中で失敗・打ち切りになった場合に未読記事が誤って既読化されることを防いだ(完全な未読スナップショットを確認できた場合のみ反映)。
