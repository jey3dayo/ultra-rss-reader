...
description: Dated triage record classifying the 27 no-high-complexity-react-function findings now in scope, 25 from Issue #249, useAccountDetailViewProps added once Issue #256's split removed the reason it was excluded, and FeedTreeRow added once Issue #324's set comparison found it unclassified, with a disposition and rationale for each and a structural family where one has been assigned.
...
cognitive figure rather than 2.5x the next one. #331 then moved its last two single-consumer
derivations to their consumers, and the scan no longer reports it; the row is marked in place.

**A 27th row was added on 2026-09-18 for `FeedTreeRow`.** It was never classified: #302 made it
report, #306 triaged the four warnings that PR added without covering it, and the arithmetic hid
that, because #306 also *fixed* `SidebarNavButton` — so the record kept 26 rows while the scan
reported 25 of them plus one with no row, and 25 = 25 looked correct. Comparing the two sets by
rule, path and function name is what found it; that comparison is Issue #324. The three rows below
whose findings the scan no longer reports are marked in place rather than deleted, because the row
is the record of the decision.

So the table below holds 27 rows, of which the scan reports 24. Its last two are dated later than
the rest.
...
| --- | --- | --- |
| must-fix | 0 | — |
| accepted-risk | 27 | Left as-is; each is an intentional single-owner surface whose split is recorded as a net loss below. The 26th is the post-split remainder of `useAccountDetailViewProps`, whose split *was* a net gain and was carried out; the 27th is `FeedTreeRow`, added by the Issue #324 set comparison |
| false-positive | 0 | — |
| suppress | 0 | — |
...
metric exceeds 15. `SidebarNavButton` (cyclomatic 15, cognitive 16) was reported on the cognitive
side alone, which is what made it the clearest example; #306 has since taken it under both
thresholds, and the example stands on the measurement rather than on the finding still existing. The rule is `severity: warn`, tagged `test-noise` and `react-jsx-only`, and its recommendation
...
Counts in the Rationale column below are stated so a later reader can reproduce them. Every count was
taken from the reported function's own body on `f9df8be7c`, not from the whole file, and uses these
units:

| Unit | Counted as | Excluded |
| --- | --- | --- |
| optional prop | a member of the component's props type alias with `?:`, at the top level of that alias | nested members inside an object-typed prop; required members |
| store selector | one `useUiStore(` or `usePreferencesStore(` call | store access through `.getState()` |
| query hook | one React Query data hook invoked directly in the body | composed hooks that wrap other hooks, such as `useArticleListSources` and `useArticleListData` |
| term of a conjunction or disjunction | one operand of the `&&` or `\|\|` chain, as written | operands of a nested expression inside one operand |
| reference to a flag | one textual occurrence of the identifier in the body, including its definition | occurrences in the props type or in module scope |
| ternary ladder arm | one result branch of a chained conditional expression | the conditions themselves |
| early return | one `return` statement that exits before the component's final return | the final return |

Where a count would be ambiguous under any of these units, the row states the unit inline instead of
...
### C. Settings-view availability chain

A settings view where feature or action availability is decided by long boolean chains over the same
flags. Two shapes occur, and `AccountSyncSectionView` contains both: a conjunction asserting that a
whole optional feature's props were supplied as a set (`AccountDangerZoneView`, and the
`on<Action> && <action>Label` pairs in `AccountSyncSectionView`), and disjunctions over shared busy
flags that gate actions (`DataSettingsView`, and `syncActionDisabled` in `AccountSyncSectionView`).
Either way the chain is one predicate that must be read as a unit, and moving it into a child only
relocates the same terms. Where the shape is prop presence, the real improvement is a props-shape
change — one optional object instead of N optional props — which is a separate design decision.
...
| `src/components/reader/feed-tree-row.tsx:69` | `FeedTreeRow` | 16 | 15 | A | accepted-risk | Classified 2026-09-18 by the Issue #324 set comparison, which is what found it unclassified; measured at `742a6919b`. Cyclomatic-dominant at nesting 1. One sidebar feed row whose optional surfaces are drag handle, favicon, unread trailing, selection indicator and context menu — but the count is dominated by `feed.isLeaving`, threaded through nine sibling guards: `canDragThisFeed`, `isCurrentSelection`, `registerSidebarNavigationTarget`, the conditional `tabIndex={-1}` spread, the leaving-marker attribute spread, and four handler gates (`onContextMenu`, `onKeyDownCapture`, `onClick`, `onMouseDown`). Those nine must agree: [../.claude/rules/motion-exit-animation.md](../.claude/rules/motion-exit-animation.md) requires a leaving row to leave the tab order, the navigation registry and every pointer path together, and the two comments in the body record why `inert` and a plain `tabIndex={undefined}` do not work here. A split that lets one guard drift leaves a departing row keyboard-reachable or focusable mid-animation, which is the defect the contract exists to prevent. |
...
| `src/components/shared/workspace-header.tsx:93` | `WorkspaceHeader` | 33 | 33 | F | accepted-risk | Three window-chrome modes (macOS overlay, compact desktop, browser preview) decide drag-region placement, eyebrow position, back-button visibility, and vertical rhythm, alongside a content-change motion effect. The two two-line comments in the body record why the drag surfaces are layered the way they are; per-platform splitting triplicates the header content. |
...
| `src/components/subscriptions-index/subscriptions-overview-summary.tsx:216` | `SummaryFilterCardButton` | 16 | 15 | A | accepted-risk | Cyclomatic-only, nesting 1. One `summaryCard.isActive` flag threaded through twelve references across seven sibling nodes (the button's `aria-pressed`, accent bar, badge, motion key and phase wrapper, value tone, caption tone, screen-reader hint) plus a review-only tooltip wrapper. Any split still needs the same flag in each piece. |
