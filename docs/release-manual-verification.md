...
- Internal-only risk may stay in `todo.txt` or its linked GitHub issue when it
  has no expected user-facing behavior and no user action can reduce the risk.
- If a tracked risk is mentioned publicly, describe the risk in user terms and
  link to stable public docs or issue references when available. Do not link
  release notes directly to `todo.txt`; it is an internal planning file and may
...
- Settings profile export copy is an outstanding requirement, not an existing fact. The shipped copy in `src/locales/*/settings.json` states only what is exported and that passwords and article data are excluded; it does not yet say that the file carries account `server_url` and `username` as private identifiers, or that it restores neither the article library nor secrets. Record this as unmet until the locale resources say it, and re-check it whenever a release touches settings profile copy.
