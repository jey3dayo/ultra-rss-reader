...
- Settings profile export copy in `src/locales/*/settings.json` says the file carries each account's server URL and username, and that it restores neither the article library nor sign-ins. Confirm both statements survive any release that touches settings profile copy; a contract test pins the substantive tokens, but reworded copy must keep the disclosure itself.
