...
また、永続化を成立させるために `src-tauri/src/commands/preference_commands.rs` の `ALLOWED_KEYS` にも同じ 3 key を追加する。
TypeScript 側だけに key を追加しても `set_preference` が `Unknown preference key` で拒否するため、frontend store と Tauri backend allowlist は同時更新を前提とする。
...
ラベル文字列は `src/locales/en/settings.json` と `src/locales/ja/settings.json` に追加し、TSX 側へハードコードしない。
...
設定変更で「現在アクティブな sidebar 由来の状態が無効になる」場合だけ、自動 fallback を行う。
有効な状態を持っている場合は何も変えない。

ここでいう active state の判定は `selection` だけでなく `viewMode` も見る。
既存の menu / keyboard / `cycle_filter` は `setViewMode("unread" | "starred")` だけで状態を切り替えるため、`selection` 単独では `Unread` / `Starred` の実際の表示状態を取りこぼす。

### active state の判定

- `Tags` が active: `selection.type === "tag"`
- `Unread` が active: `selection.type === "smart" && selection.kind === "unread"` または `viewMode === "unread"`
- `Starred` が active: `selection.type === "smart" && selection.kind === "starred"` または `viewMode === "starred"`

### fallback ルール

1. 現在が `Starred` で、かつ `selection.type === "smart"` 由来の場合は `Unread` へ移動する
2. 現在が `Starred` で、`viewMode === "starred"` のみで表現されている場合は、現在の `selection` を維持したまま `viewMode = "all"` に戻す
3. 現在が `Tag` で、`show_sidebar_tags = false` になった場合は `Unread` へ移動する
4. 現在が `Unread` で、かつ `selection.type === "smart"` 由来の場合は `Feeds` 側へ移動する
5. 現在が `Unread` で、`viewMode === "unread"` のみで表現されている場合は、現在の `selection` を維持したまま `viewMode = "all"` に戻す
6. `Unread` へ移動したいが `show_sidebar_unread = false` の場合は、現在アカウント内の feed selection へ移す
7. 明示的な feed selection が作れない場合は、現在アカウントの先頭 feed か既存 safe default にフォールバックする
...
ただし sidebar 上で現在どの smart view が実質 active かの判定は `selection` と `viewMode` の両方を見る。
今回の変更では selection 型自体は増やさず、`sidebar.tsx` 側で「この state は今の設定で有効か」を判断して、必要時のみ既存 action や `setViewMode("all")` を呼ぶ。
...
`src-tauri/src/commands/preference_commands.rs`

- `set_preference` が新しい 3 key を `Unknown preference key` にせず受け付けること
...
- `selection` 由来の `Starred` / `Tag` は `Unread`、`Unread` は `Feeds` 側へ fallback すること
- `viewMode` のみで active な `Unread` / `Starred` も取りこぼさず `viewMode = "all"` へ戻すこと
...
`sidebar.tsx` で設定変更を監視して state を動かす際、条件が広すぎると再レンダリングごとに `selectSmartView` / `selectFeed` / `setViewMode` を呼び続ける危険がある。
そのため fallback は「現在の active state が無効になった瞬間」に限定する必要がある。
