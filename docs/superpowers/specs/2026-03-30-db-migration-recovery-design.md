...
6. バックアップがあれば restore を試みる
7. restore 成功後も起動を継続せず、migration failure として停止する
...
If you need to restore manually, close the app and replace the database file with the backup file.
...
- migration recovery 用メッセージにバックアップ場所が含まれる
- migration recovery 用メッセージに手動リストアの一般手順が含まれる
