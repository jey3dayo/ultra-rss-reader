...
- `src-tauri/src/lib.rs` の `tauri::generate_handler!` にも新 command を登録し、`invoke("count_account_unread_articles")` が解決される状態にする
...
加えて `src-tauri/src/lib.rs` の `tauri::generate_handler!` に `commands::article_commands::count_account_unread_articles` を登録する。これを省くとフロント側の wrapper 追加だけでは `unknown command` になる。
...
- 同じ query key を共有する将来の feed/account 追加・削除フロー

## Error Handling

- Rust command は既存 read command と同じ `AppError` 変換方針に従う
- `useBadge` は新 query の失敗時でも UI 全体を壊さない。ただし `only_inbox` の件数 query が失敗した場合はバッジ更新をスキップせず、`setBadgeCount(undefined)` で明示的にクリアする
- Tauri 以外の実行環境で `setBadgeCount` が失敗しても、現行どおり no-op とする

これは React Query が直前の成功値を保持したまま error 状態になる可能性を考慮したもの。アカウント切替や refetch 後に stale な件数が Dock/Taskbar に残らないことを優先する。
...
- unread count query が失敗した場合はバッジがクリアされ、hook もクラッシュしない
