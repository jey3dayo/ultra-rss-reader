...
- [ ] アプリ終了・クラッシュ後に戻りやすくするため、最近見た記事リストを追加する
  ...
  - アカウント単位で直近 50 件程度を保持し、同じ記事を再表示した場合は先頭へ移動する
  ...
  - 2026-04-27 時点の dev アプリ手動確認では、`一般 > サイドバー` の `最近見た記事を表示` と、`閲覧` の履歴関連設定が表示されない再現がある
  - 該当ブランチ: `codex/recent-article-history`
  - 自動テストだけでは検出できていないため、設定モーダルとスマートビューの表示確認テストを追加して回帰を止める
