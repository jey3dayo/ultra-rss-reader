...
- [ ] P2 bulk article mutation の missing / mixed-account ID policy を固定する
  ...
  - missing id、mixed account、deleted feed、partial update count、rollback/refetch policy の test を追加する
...
- [ ] P1 mute keyword 追加/更新 race と duplicate 正規化を UI/backend で一本化する
...
- [ ] P2 dev credential store lock の stale lock recovery 方針を決める
  - 対象: `src-tauri/src/infra/keyring_store.rs`, `src-tauri/src/commands/account_commands.rs`
  - dev credentials の lock file が異常終了で残ると timeout で止まるだけになり、開発環境復旧手順が属人化しやすい
  - stale pid、mtime timeout、manual cleanup message、concurrent writer、corrupted lock file の Rust test を追加する
...
- [ ] P2 old unread count/mark の `Utc::now()` 境界 drift を固定する
  - 対象: `src-tauri/src/commands/article_commands.rs`, `src/components/reader/hooks/feed-actions/use-old-unread-read-action.ts`
  - count と mark が別 command でそれぞれ `now` を取るため、境界時刻付近の記事で確認件数と実 mark 件数がずれる可能性がある
  - fixed clock injection、count/mark shared cutoff、article exactly at cutoff、DST/timezone、confirmation copy の test を追加する

- [ ] P2 old unread の `published_at` 文字列比較を timestamp normalization contract にする
  - 対象: `src-tauri/src/commands/article_commands.rs`, `src-tauri/src/infra/db/sqlite_article.rs`
  - SQL が `published_at < ?` の文字列比較なので、fractional seconds、timezone offset、invalid legacy value の ordering が暗黙
  - RFC3339 nano、offset timezone、invalid string、null legacy、migration repair の Rust test を追加する
...
- [ ] P1 tag create + assign の two-step partial success policy を決める
  - 対象: `src/components/article/article-tag-chips.tsx`, `src/hooks/use-tags.tsx`, `src-tauri/src/commands/tag_commands.rs`
  - tag 作成成功後に article への assignment が失敗すると、tag だけ残って未紐付けになる状態の UI/再試行方針が未契約
  - create success assign failure、retry assign、newTagName reset、picker close、toast copy の test を追加する
...
- [ ] P2 createQuery diagnostics の raw queryId redaction を追加する
  - 対象: `src/hooks/create-query.ts`, `src/lib/runtime/diagnostics.ts`
  - error message/diagnostic に raw `queryId` が入り、将来 URL/token/private id を query id にした時に漏洩し得る
  - URL id、token-like id、account id、disabled refetch、redaction snapshot の test を追加する

- [ ] P2 createMutation が error classification を保持するようにする
  - 対象: `src/hooks/create-mutation.ts`, `src/api/schemas/error.ts`, `src/lib/ui-errors.ts`
  - `Result.fail` が `Error(message)` に潰されると、category/user-visible/diagnostics/locale key の情報を失う
  - AppError failure、schema failure、runtime failure、toast category、mutation error surface の test を追加する
...
- [ ] P2 runtime diagnostics once suppression set growth を `once=false` で抑える
  - 対象: `src/lib/runtime/diagnostics.ts`
  - `once:false` の diagnostic でも once key を set に追加しており、高カーディナリティ detail で grow し続ける可能性がある
  - once false repeated details、memory cap、reset helper、background spam、policy test を追加する
...
- [ ] P2 sidebar expanded folders storage schema の prototype key hardening を行う
  - 対象: `src/schemas/storage.ts`
  - localStorage 由来の account id を plain object に入れるため、`__proto__`/`constructor` key の扱いを明文化していない
  - proto key、constructor key、null-prototype output、JSON parse、account cap の test を追加する

- [ ] P2 storage/preference control char policy を揃える
  - 対象: `src/schemas/storage.ts`, `src/schemas/preferences.ts`
  - storage は C0/DEL 除去、preferences は C1 も reject するなど、control char の normalize/reject 方針が drift している
  - C0、DEL、C1、normalize vs reject、migration/cleanup の test を追加する
...
- [ ] P1 feed 単体 sync 成功時に account scheduler backoff を消す契約を分ける
  - 対象: `src-tauri/src/commands/sync_commands.rs`, `src-tauri/src/service/sync_scheduler.rs`
  - `trigger_sync_feed` 成功で account 全体の scheduler status を clear すると、同一 account の別 feed に残る failure backoff まで消し得る
  - scheduler error state、feed A single sync success、feed B failure retained、account sync success clears、feed sync warning retains status の test を追加する
...
- [ ] P2 scheduler retry-scheduled warning と provider warning の dedupe/cap を固定する
  - 対象: `src-tauri/src/service/sync_scheduler.rs`, `src/lib/sync/sync-result-feedback.ts`
  - 失敗ごとに RetryScheduled warning を積むと provider warning と重なり、background failure で UI/toast が騒がしくなる可能性がある
  - retry warning dedupe、provider warning + retry warning、same account repeated tick、multi-account warning cap、retry_at更新時のみ通知 の test を追加する
...
- [ ] P2 `add_local_feed` の fetch 待ち中 account drift を固定する
  - 対象: `src-tauri/src/commands/feed_commands.rs`
  - account kind/local 存在確認後に外部 fetch を待って保存するため、待機中の account 削除・kind 変更時の error/rollback が揺れやすい
  - account deleted after validation、account kind changed、FK failure message、no orphan feed、retry after account recreated の test を追加する

- [ ] P2 OPML import の duplicate URL skip を URL 正規化単位で固定する
  - 対象: `src-tauri/src/commands/opml_commands.rs`
  - duplicate 判定が保存済み URL との文字列一致に寄ると、scheme/host case、default port、trailing slash、percent encoding 差で重複登録し得る
  - host case、default port、trailing slash、query order、percent-encoded path、existing normalized local feed の test を追加する
...
- [ ] P2 dev credential store の account id key validation を DB id contract と揃える
  - 対象: `src-tauri/src/infra/keyring_store.rs`, `src-tauri/src/commands/account_commands.rs`
  - dev JSON store は account id をそのまま object key にするため、空文字/control char/巨大 id/特殊 key を credential boundary でどう扱うか未契約
  - empty account id、control char、very long id、`__proto__`-like key、normal generated id、missing password error の Rust test を追加する

- [ ] P2 keyring force-delete fallback warning の stderr redaction を追加する
  - 対象: `src-tauri/src/infra/keyring_store.rs`
  - macOS `security` fallback の stderr を warning に含めるため、将来 command stderr に user path/account detail が混ざった時に log privacy boundary が弱くなる
  - stderr with account id、user path、quoted service、non-UTF8 stderr、status only message の test を追加する

- [ ] P2 dev credential lock file の stale owner diagnostics を redacted にする
  - 対象: `src-tauri/src/infra/keyring_store.rs`
  - lock timeout message が lock path をそのまま出すため、HOME/user name や temp dir layout が diagnostics/log に残る
  - timeout path redaction、relative path、Windows path、permission error、support log copy の test を追加する

- [ ] P2 `open_log_dir` の non-UTF8 path / opener argument policy を固定する
  - 対象: `src-tauri/src/commands/log_commands.rs`
  - `dir.to_string_lossy()` を opener に渡すため、non-UTF8 path を含む環境で開く場所や error message が曖昧になる
  - non-UTF8 app_log_dir、lossy path warning、open failure、redacted path、user-visible recovery copy の Rust test を追加する

- [ ] P2 `open_log_dir` の symlink / file collision handling を固定する
  - 対象: `src-tauri/src/commands/log_commands.rs`
  - log dir path に既存 file や symlink がある場合、create/open の error surface と support message が OS ごとにずれやすい
  - existing file at log dir、symlink dir、permission denied、create_dir_all race、opener failure の test を追加する
...
- [ ] P2 platform info load failure の retry trigger と stale in-flight を固定する
  - 対象: `src/stores/platform-store.ts`, `src/__tests__/stores/platform-store.test.ts`
  - load failure 後は retry 可能だが、in-flight 中の runtime recovery や stale failure が最新 success を上書きしない契約が薄い
  - first failure then retry success、success then stale reject、parallel callers、runtime unavailable、diagnostics once の test を追加する

- [ ] P2 always-on-top preference failure の UI state rollback / notification policy を決める
  - 対象: `src/hooks/use-window-always-on-top.ts`, `src/stores/preferences-store.ts`, `src/lib/window/windows.ts`
  - preference は optimistic に保存されるが native `setAlwaysOnTop` が失敗した時に UI state を戻すか、toast/log のみかが未契約
  - unsupported error、permission failure、latest-only rapid toggle、persist success/native failure、setting reload の test を追加する

- [ ] P2 window API error message sanitizer を throwing `message` getter / Symbol で固定する
  - 対象: `src/lib/window/windows.ts`, `src/__tests__/lib/windows.test.ts`
  - window helper は unknown error を `Reflect.get(error, "message")` で読むため、throwing getter や Symbol/stringify failure の扱いを regression 化したい
  - throwing message getter、symbol error、object without message、toString throws、known Error passthrough の test を追加する

- [ ] P2 preference typo detection の O(n*m) scan を large passthrough key で上限化する
  - 対象: `src/schemas/preferences.ts`, `src/stores/preferences-store.ts`
  - unknown preference key ごとに edit distance を全候補へかけるため、壊れた backend/import で大量 key が来ると DEV 起動が重くなり得る
  - 1k unknown keys、very long key、reserved prefix、retired key、diagnostics cap の test を追加する

- [ ] P2 preference `normalizePreferenceRecord` の prototype pollution safe output を固定する
  - 対象: `src/schemas/preferences.ts`, `src/stores/preferences-store.ts`
  - `Object.fromEntries` で backend passthrough key を保持するため、`__proto__` / `constructor` などの key を state に入れる policy を明文化する
  - `__proto__` key、constructor key、hasOwnProperty shadow、null-prototype output、resolvePreferenceValue の test を追加する

- [ ] P2 feed discovery response decoding の lossy UTF-8 policy を固定する
  - 対象: `src-tauri/src/infra/feed_discovery.rs`
  - `String::from_utf8_lossy` で HTML/feed body を読むため、invalid UTF-8 を含む feed discovery で fallback/feed link extraction が壊れても気づきにくい
  - invalid UTF-8 HTML、invalid UTF-8 XML、replacement char in href/title、content-type charset、user-visible error vs lossy accept の test を追加する

- [ ] P2 feed discovery `<base>` href の path traversal / origin policy を contract 化する
  - 対象: `src-tauri/src/infra/feed_discovery.rs`
  - `<base>` は same-origin のみ許可しているが、path/query/fragment や protocol-relative base の扱いが feed URL 解決に影響する
  - same-origin absolute base、protocol-relative base、base with query/fragment、parent path、invalid base fallback の test を追加する

- [ ] P2 feed discovery duplicate feed link を normalized final URL で dedupe するか決める
  - 対象: `src-tauri/src/infra/feed_discovery.rs`
  - `seen_urls` は resolved URL 文字列基準なので、fragment、case、default port、percent encoding 差で同じ feed 候補が複数表示され得る
  - fragment difference、host case、default port、trailing slash、percent encoding、query order の test を追加する
...
- [ ] P3 shortcuts category empty/unknown label fallback を locale contract にする
  - 対象: `src/components/settings/hooks/use-shortcuts-settings-view-props.ts`, `src/locales/ja/reader.json`, `src/locales/en/reader.json`
  - category label は `tReader(category)` に直渡しなので、新 category 追加時の missing locale が UI に key 表示として出やすい
  - missing category locale、new category fixture、empty definitions、category order、locale parity の test を追加する

- [ ] P3 FeedFavicon fallback label の grapheme / CJK / emoji policy を決める
  - 対象: `src/components/shared/feed-favicon.tsx`, `src/__tests__/components/feed-favicon.test.tsx`
  - `charAt(0).toUpperCase()` は surrogate pair や combining mark を分割し、CJK/emoji title の fallback 表示が崩れる可能性がある
  - emoji title、combining mark、CJK title、blank title、RTL title、unknown title fallback の test を追加する
...
- [ ] P1 GReader unread reconcile の pending mutation 判定を axis 別にする
  ...
  - pending star only、pending read only、both pending、remote unread surplus、local surplus の reconcile test を追加する

- [ ] P1 pending mutation の対象 feed 判定を `remote_entry_id` 衝突で固定する
...
- [ ] P2 GReader cursor advance を provider `next_cursor` 欠落時の再取得で固定する
  - 対象: `src-tauri/src/commands/sync_providers.rs`
  - state advance が `next_cursor.since` 依存のため、entries は返るが cursor がない provider 応答で同じ差分を繰り返す可能性がある
  - entriesありnext_cursorなし、has_more false、multi page cursor欠落、saved timestamp据え置き時の再sync test を追加する

- [ ] P2 remote-state cooldown を future timestamp / clock rollback で固定する
  - 対象: `src-tauri/src/commands/sync_providers.rs`
  - `last_success_at` が未来だと remote state pull が長時間 skip され、端末時刻変更後の sync が古い状態に固定され得る
  - future timestamp、clock rollback、malformed timestamp、exact cooldown boundary、timezone offset の test を追加する

- [ ] P2 日付なし provider entry の `published_at` fallback を安定化する
  - 対象: `src-tauri/src/commands/sync_providers.rs`, `src-tauri/src/infra/provider/normalizer.rs`
  - missing published/updated が毎回 `Utc::now()` になると、既存記事の並びが sync のたびに動く可能性がある
  - same remote id with missing dates、second sync ordering、fetched_at update、manual feed sync/account sync parity の test を追加する

- [ ] P2 normalizer の whitespace-only entry id を blank として扱う
  - 対象: `src-tauri/src/infra/provider/normalizer.rs`
  - `entry.id.is_empty()` だけだと whitespace が remote id として保存され、URL fallback や pending mutation の前提が崩れやすい
  - empty id、whitespace id、trimmed id、duplicate whitespace id、URL fallback id generation の test を追加する

- [ ] P2 article URL credential stripping policy を security contract にする
  - 対象: `src-tauri/src/infra/provider/normalizer.rs`, `src/lib/feed/feed.ts`
  - metadata URL は credential を拒否する一方で article URL は credential を削除して保存するため、private URL の扱いが provider 間でずれる
  - username/password URL、fragment付きURL、metadata/article URL parity、UI表示/log redaction の test を追加する

- [ ] P2 local feed 304 sync 後の mute auto-mark / count repair contract を作る
  - 対象: `src-tauri/src/commands/sync_providers.rs`, `src-tauri/src/infra/db/sqlite_article.rs`
...
- [ ] P2 sanitizer reprocess で summary fallback search text を失わない契約を決める
  ...
  - `update_sanitized` が summary を渡さない場合、empty sanitized の記事が summary 検索・mute body 判定から落ち得る
  - empty sanitized with summary、sanitizer migration、search by summary、mute body by summary、existing content_text overwrite の test を追加する
...
- [ ] P2 Local account command args の blank credential policy を FreshRSS と揃える
  - 対象: `src/api/schemas/commands.ts`, `src-tauri/src/commands/account_commands.rs`
  - Local account の `serverUrl/appId/appKey/username/password` が trim/blank policy の差で whitespace だけ backend に渡り得る
  - Local blank credentials、whitespace serverUrl、FreshRSS parity、optional omitted fields、schema/frontend error の test を追加する

- [ ] P2 feed discovery/add command URL schema を browser/open URL schema と揃える
  - 対象: `src/api/schemas/commands.ts`, `src-tauri/src/infra/feed_discovery.rs`, `src-tauri/src/commands/feed_commands.rs`
  - `discoverFeedsArgs` / `addLocalFeedArgs` が nonblank string 中心だと、invalid URL failure surface が backend 依存になりやすい
  - ftp/javascript/mailto/newline/leading whitespace、frontend schema failure、backend UserVisible failure の test を追加する

- [ ] P2 external URL schema の uppercase scheme policy を command 間で揃える
  - 対象: `src/api/schemas/commands.ts`, `src/lib/actions.ts`, `src-tauri/src/commands/article_commands.rs`
  - `externalUrlSchema` は `startsWith` の case-sensitive 判定で、HTTP URL schema と uppercase/mixed-case scheme の扱いがずれ得る
  - uppercase scheme、mixed-case mailto、trimmed URL、newline redaction、open external/browser parity の test を追加する
