...
- [ ] 右ペインを `Web プレビューモード` として明確にする
  - 画像重視の viewer を維持しつつ、最低限の文脈情報を固定ヘッダで補う
  - 候補情報: 記事タイトル / フィード名 / 投稿日 / 既読状態 / 外部ブラウザで開く / 前後の記事移動
  - 「記事本文なのか」「Web preview なのか」が見た目で判断できる状態を目指す
- [ ] 左ペインの現在地をもう一段分かりやすくする
...
- [ ] ツールバーとメニューバーの役割を整理する
  ...
  - overlay 中と通常時で、close 導線の責務を混ぜない
- [ ] UI 言語の混在を解消する
  - 同一画面で日本語 UI と英語 UI が混在しないようにする
  - まずは主要画面で、日本語向けなら日本語に寄せる方針を確認する
- [ ] 表示モードの整理方針を決める
  - `Reader mode` と `Web preview mode` の 2 軸を第一候補として整理する
  - `Media mode` のような追加モードは、必要性が出た時点で改めて検討する
