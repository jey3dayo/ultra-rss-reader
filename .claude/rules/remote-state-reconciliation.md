...
per-feed の pending 保護と最終 recalculate が担う。不完全な snapshot でもローテーションの
チェック時刻は進める。構造的な不完全は即時再試行で解決しないためであり、記録しないと
慢性的に不完全な feed が毎回3枠のうち1枠を占有して他の feed が飢餓になる。したがって、
慢性的に不完全な snapshot を返す feed は実質的に drift check されないままになるという
既知の限界がある。
