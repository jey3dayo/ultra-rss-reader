...
- **Optional-prop capability gate.** A settings view enabling a whole optional feature through a long conjunction that asserts its props were supplied as a set. The conjunction is one predicate. The real improvement is a props-shape change (one optional object instead of N optional props), which is a separate design decision, not a complexity fix.
