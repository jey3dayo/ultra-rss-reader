...
are manual. Verified 2026-09-10 against the primary sources, and still true: `lefthook.yml` runs
...
Measured 2026-09-19 at `6e967f4b1`, four facts the heuristics above do not cover.

**Hub-shaped noise is not per-pair noise.** `similarity-ts --threshold 0.9 src/` reported 40 function pairs, and `useBrowserWebviewBoundsSync` (`use-browser-webview-bounds-sync.ts`) appears in 9 of them, paired with unrelated targets including `patchCachedAccount`, `useSubscriptionsIndexEscape`, and `useAccountDetailSyncStatusRows`. The eight most frequent symbols fill 44 of the 80 pair slots. Triage should start from how many pairs a symbol appears in, not from any one pair's percentage. Decision taken 2026-09-19: add no suppression mechanism, because a hub-level allowlist would hide a genuine duplication later appearing on the same hub, and `similarityScanBaseline` is display-only rather than a gate.
...
**The scan cannot see the repository's largest duplication.** `tests/helpers/deferred.ts` already exported `createDeferred`, yet 22 files under `src/__tests__` defined their own copy while 4 imported the shared one. The scan reported none of it: `--filter-function createDeferred --threshold 0.9` returns 0 pairs because the bodies are around 8 lines and the size penalty sinks them; with `--no-size-penalty` the same filter returns 1478. A green similarity report is therefore not evidence that shared helpers are being used. `tests/` is also outside the scan's `src/` root, so a local copy is never compared against the shared original. The 22 copies were removed in commit `1b9bf2bb3`. Measurement caution: the first inventory counted 23 files and 26 definitions and both were wrong. `grep -E "^[[:space:]]*(const|function) createDeferred"` matched `createDeferredCleanup()` in `tauri-event-listeners.node.test.ts` (a different helper returning `{ resolveCleanup, subscription }`, correctly left alone) and matched call sites of the form `const createDeferredResult = createDeferred<...>()`. The strict pattern is `^[[:space:]]*(const|function) createDeferred[<(]`. Do not count helpers by name prefix.
