...
- Utility Icon Actions: Compact icon-only support actions in desktop chrome should rest in a borderless, transparent state. In sidebar headers, article toolbars, browser-overlay rails, and similar utility-action strips, express affordance through hit area, icon contrast, and hover/focus surface treatment before introducing a persistent outline. Selected or toggled states should step up through tonal fill only when the active state is a meaningful emphasis; do not turn a baseline state such as read/default/closed into a filled selected surface just because the control has `aria-pressed`. Use semantic icon tint only when the state itself is semantic, such as starred or unread. Keep focus as a separate layer so keyboard emphasis is clearer than selected state.
