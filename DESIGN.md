...
- Settings Borders and Shadows: Settings should follow a low-wire desktop preferences model, closer to Inoreader-style grouped preferences than card-heavy web forms. The outer modal shell may carry the visible frame and ambient shadow, but the content pane should avoid repeated card outlines. Right-pane settings should read as section label plus a quiet rounded row group: group separation comes from surface tone, row cadence, and subtle internal dividers, not from heavy outer borders around every section. Use `--settings-shell-divider-border` for shell/header/nav and row dividers, `--settings-shell-section-border` only as a nearly invisible group edge, and `--settings-shell-control-border` for controls that still need a boxed affordance. Use `--settings-shell-shadow` for the outer modal shell; keep `--settings-shell-section-shadow` and `--settings-shell-control-shadow` very quiet or none so sections, selects, shortcut key badges, and content/header action buttons do not compete with the shell. Avoid arbitrary per-section shadows or stacking multiple adjacent borders where a single divider or tonal group can carry the separation.
