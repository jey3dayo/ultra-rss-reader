...
fn create_mute_keyword_rolls_back_rule_when_auto_read_fails() {
    ...
    let error = create_mute_keyword_impl(&db, "Kindle Unlimited".to_string(), "title".to_string())
        .unwrap_err();
...
fn update_mute_keyword_rolls_back_scope_when_auto_read_fails() {
    ...
    let error = update_mute_keyword_impl(&db, created.id.clone(), "title".to_string()).unwrap_err();
