...
describe("test fixtures", () => {
  ...
  it("keeps sample article, feed, account, and tag references internally consistent", () => {
    ...
    expect(
      sampleArticleTags.every((articleTag) =>
        articleIds.has(articleTag.article_id),
      ),
    ).toBe(true);
    expect(
      sampleArticleTags.every((articleTag) => tagIds.has(articleTag.tag_id)),
    ).toBe(true);
  });
  ...
  it("keeps sample feed folder references resolvable within the same account", () => {
    ...
    const folderCapableFeeds = sampleFeeds.filter(
      (feed) =>
        accountsById.get(feed.account_id)?.capabilities?.supports_folders ===
        true,
    );

    expect(folderedFeeds.length).toBeGreaterThan(0);
    expect(folderCapableFeeds.some((feed) => feed.folder_id !== null)).toBe(
      true,
    );
    expect(folderCapableFeeds.some((feed) => feed.folder_id === null)).toBe(
      true,
  ...
  it("keeps required fixture identity and display fields populated", () => {
    for (const account of sampleAccounts) {
      ...
      expectNonBlank(
        ...
        `account ${account.id} display_name is required`,
      );
    }
    ...
    for (const article of sampleArticles) {
      ...
      expectNonBlank(
        article.feed_id,
        `article ${article.id} feed_id is required`,
      );
      ...
      expectNonBlank(
        ...
        `article ${article.id} published_at is required`,
      );
    }
    ...
    for (const articleTag of sampleArticleTags) {
      expectNonBlank(
        ...
        "article tag article_id is required",
      );
