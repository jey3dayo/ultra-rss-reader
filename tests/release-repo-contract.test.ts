...
const RELEASE_UPDATER_ENDPOINT =
...
const MOBILE_ICON_ASSET_HASHES = {
  ...
  "icons/android/mipmap-hdpi/ic_launcher.png":
    "75ab910077e745762b3c19ce91ab6606b48bf48749d9c643baf0567cd1d17c10",
  ...
  "icons/android/mipmap-hdpi/ic_launcher_round.png":
    "5e6c92a894078957666b90f2fc8efb8c9e95c9d16a0ede6098d7f627ea2025d1",
  "icons/android/mipmap-mdpi/ic_launcher.png":
    "3ce45ae095e68d72c07e865bd621e8bd3ce9d3799f44f33ce15f749825a9ab56",
  ...
  "icons/android/mipmap-mdpi/ic_launcher_round.png":
    "e4d23648b41f28e6067f49c727c950acec80a7df86d1355dbe9553d0003bfbc6",
  "icons/android/mipmap-xhdpi/ic_launcher.png":
    "55aa6f966e75796a16a3efd619541cd0d11eceed1c509e1af40b21eba2f91948",
  ...
  "icons/android/mipmap-xxhdpi/ic_launcher.png":
    "6e31d70ff9ef085d023bed8464dbb458670bbdd9a6ad495e04a2ce0c82e804c0",
  ...
  "icons/android/mipmap-xxxhdpi/ic_launcher.png":
    "d0fd2ff0daa1677f55287401ae78ffe6535adf59995b536671405bdea733bf44",
  ...
  "icons/ios/AppIcon-20x20@1x.png":
    "c8f9dc853b01c5ad89f7c22f3e28558eadfc5898389484a730a19344d2ab06cd",
  "icons/ios/AppIcon-20x20@2x-1.png":
    "12927bc96a63bd13dd79d792e8625388d57061325c34df28049207b2600a61a3",
  "icons/ios/AppIcon-20x20@2x.png":
    "12927bc96a63bd13dd79d792e8625388d57061325c34df28049207b2600a61a3",
  "icons/ios/AppIcon-20x20@3x.png":
    "7845d6efbdb6eba1a66bd5b683c35c5153e6a2132fab3c2634e128bd2514183a",
  "icons/ios/AppIcon-29x29@1x.png":
    "16195adc8513968986255c86f0b9a39493a3dc1ca10a766b877a175d66652087",
  "icons/ios/AppIcon-29x29@2x-1.png":
    "b81826b2a42b24cbc26b63144ce9ebabeb2ea345a3e3a47a4f6d8e861dd16429",
  "icons/ios/AppIcon-29x29@2x.png":
    "b81826b2a42b24cbc26b63144ce9ebabeb2ea345a3e3a47a4f6d8e861dd16429",
  "icons/ios/AppIcon-29x29@3x.png":
    "3e0ed19c4c230e7c36963ea18336e4d4f73d72b07e8633f58b67f254a92e2b02",
  "icons/ios/AppIcon-40x40@1x.png":
    "12927bc96a63bd13dd79d792e8625388d57061325c34df28049207b2600a61a3",
  "icons/ios/AppIcon-40x40@2x-1.png":
    "220be6e8e4b497466eeb0a0ed144874192c662dcd4d19390c89b89bb349454d8",
  "icons/ios/AppIcon-40x40@2x.png":
    "220be6e8e4b497466eeb0a0ed144874192c662dcd4d19390c89b89bb349454d8",
  "icons/ios/AppIcon-40x40@3x.png":
    "594e35e40cba71034cdefef0b2060dfc921bbbd0b5334e35a329db6b6177027a",
  "icons/ios/AppIcon-512@2x.png":
    "780eb646c0e30e0ea75cf155814fa6c2638ba48c6d9ff496b06775832084c88f",
  "icons/ios/AppIcon-60x60@2x.png":
    "594e35e40cba71034cdefef0b2060dfc921bbbd0b5334e35a329db6b6177027a",
  "icons/ios/AppIcon-60x60@3x.png":
    "29a72741769156adb566a64f0f078b70c17affbbef5ebf30cee3786431733073",
  "icons/ios/AppIcon-76x76@1x.png":
    "02184e4fbdf23b4f3c09508f1b40492f81b0081ba3e3ab5652cb23704347bbd8",
  "icons/ios/AppIcon-76x76@2x.png":
    "2af04f10ae235a52702a9ce45f4ea2f079c7bc2f0565d96d92a78fe039bba0e1",
  "icons/ios/AppIcon-83.5x83.5@2x.png":
    "3e5a08f0e1cc8f24d56e6798855131b07e02fad99f60144c516f93ec03a5e9b7",
} as const;
const UPDATER_PUBKEY_PLACEHOLDER_PATTERN =
  /(?:placeholder|change[_-]?me|todo)/i;
...
const UNSUPPORTED_UPDATER_PLATFORM_KEYS = [
  "linux-x86_64",
  "linux-aarch64",
] as const;
...
const readSha256 = (path: string): string =>
...
const decodeTomlBasicString = (value: string): string =>
  value.replace(
    /\\(u[0-9a-fA-F]{4}|U[0-9a-fA-F]{8}|["\\btnfr])/g,
    (match, token: string) => {
      if (token === "b") return "\b";
      if (token === "t") return "\t";
      if (token === "n") return "\n";
...
const extractTomlString = (source: string, key: string): string => {
  ...
  for (let index = 0; index < lines.length; index += 1) {
    ...
    if (rest.startsWith('"""')) {
      return decodeTomlBasicString(
        readTomlMultilineString(lines, index, rest, '"""'),
      );
    }
    if (rest.startsWith("'''")) {
...
const extractCacheBlocks = (source: string): string[] => {
  const cachePattern =
    /- uses: actions\/cache@[^\n]+\n(?<block>(?: {8}.+\n?)*)/g;
  return [...source.matchAll(cachePattern)].map(
...
const capabilityByIdentifier = (
  source: TauriCapabilityFile,
  identifier: string,
): TauriCapability => {
  const capability = normalizeCapabilities(source).find(
...
const extractRustStringConstants = (
  source: string,
  suffix: string,
): Map<string, string> => {
  const constants = new Map<string, string>();
  const pattern = new RegExp(
    `^const\\s+([A-Z0-9_]+${escapeRegExp(suffix)}):\\s*&str\\s*=\\s*"([^"]+)";`,
    "gm",
  );
...
const extractNativeMenuActionContracts = (
  source: string,
): Map<string, string> => {
...
const extractAppActions = (source: string): Set<string> => {
  const registry = source.match(
    /export const APP_ACTION_REGISTRY = \{(?<body>[\s\S]*?)\n\} as const;/,
  )?.groups?.body;
  if (!registry) {
...
const extractShortcutActionIds = (source: string): Set<string> => {
  const definitions = source.match(
    /export const shortcutDefinitions: ShortcutDefinition\[] = \[(?<body>[\s\S]*?)\n\];/,
  )?.groups?.body;
  if (!definitions) {
...
const extractRustEnumVariants = (
  source: string,
  enumName: string,
): string[] => {
  const body = source.match(
    new RegExp(
      `pub enum ${escapeRegExp(enumName)} \\{(?<body>[\\s\\S]*?)\\n\\}`,
    ),
  )?.groups?.body;
  if (!body) {
...
const extractYamlScalarBlockEntries = (
  block: string,
...
const normalizeRepoPath = (path: string): string =>
...
const isMarkdownlintIgnoredPath = (path: string): boolean => {
  ...
  return (
    ...
    markdownlintRepoContract.generatedMarkdownIgnorePatterns.some(
      (pattern) =>
        pattern === "src-tauri/gen/**" &&
        normalizedPath.startsWith("src-tauri/gen/"),
    )
  );
};
...
const parseMigrationFileName = (
  ...
  const match = fileName.match(
    /^V(?<version>\d+)__(?<description>[a-z0-9]+(?:_[a-z0-9]+)*)\.sql$/,
  );
  if (!match?.groups) {
...
const migrationOwnerForDescription = (description: string): string => {
  const owner = MIGRATION_OWNER_BY_DESCRIPTION_PATTERN.find(([pattern]) =>
...
const hasDestructiveMigrationSql = (source: string): boolean => {
  const withoutSchemaVersionMetadata = source.replace(
    /DELETE\s+FROM\s+schema_version\b[^;]*;/gi,
    "",
  );
  return /\b(?:DROP|DELETE\s+FROM|TRUNCATE)\b/i.test(
    withoutSchemaVersionMetadata,
  );
};
...
describe("release repository contract", () => {
  ...
  const tauriReleaseConfig: TauriConfig = JSON.parse(
    readText("src-tauri/tauri.release.conf.json"),
  );
  const tauriDevConfig: TauriConfig = JSON.parse(
    readText("src-tauri/tauri.dev.conf.json"),
  );
  const defaultCapability: TauriCapabilityFile = JSON.parse(
    readText("src-tauri/capabilities/default.json"),
  );
  const cargoToml = readText("src-tauri/Cargo.toml");
  const releaseWorkflow = readText(".github/workflows/release.yml");
  const tauriLib = readText("src-tauri/src/lib.rs");
  const updaterCommandsSource = readText(
  ...
  const releaseManualVerification = readText(
  ...
  const prInsightsLabelerWorkflow = readText(
  ...
  const keyboardShortcutsSource = readText(
    "src/lib/keyboard/keyboard-shortcuts.ts",
  );
  ...
  const providerHttpDefaultsSource = readText(
    "src-tauri/src/infra/provider/http_defaults.rs",
  );
  ...
  const addAccountServicesSource = readText(
    "src/components/settings/add-account/services.ts",
  );
  ...
  it("parses TOML string values with quoted and multiline forms used by release contracts", () => {
    ...
    expect(extractTomlString(toml, "description")).toBe(
      "A Tauri-based RSS reader\nwith release provenance\n",
    );
  ...
  it("checks release source and version parity before artifact creation", () => {
    ...
    expect(releaseWorkflow).toContain(
      "tag push ref $WORKFLOW_REF does not match release tag $RELEASE_TAG",
    );
    expect(releaseWorkflow).toContain(
      "manual dispatch ref $WORKFLOW_REF does not match release tag $RELEASE_TAG",
    );
    expect(releaseWorkflow).toContain(
    ...
    expect(releaseWorkflow).toContain(
      'git merge-base --is-ancestor "$tag_target_sha" refs/remotes/origin/main',
    );
  ...
  it("keeps release draft and prerelease flags derived from the semver tag policy", () => {
    ...
    expect(tauriActionBlock).toContain(
      "releaseDraft: $" + "{{ steps.release-policy.outputs.draft }}",
    );
    expect(tauriActionBlock).toContain(
      "prerelease: $" + "{{ steps.release-policy.outputs.prerelease }}",
    );
    expect(tauriActionBlock).not.toContain("prerelease: false");
    expect(
      releaseWorkflow.indexOf("Resolve release semver policy"),
    ).toBeLessThan(releaseWorkflow.indexOf("tauri-apps/tauri-action"));
  });
  ...
  it("keeps CI pnpm cache restore keys bounded by frozen lockfile installs", () => {
    ...
    expect(ciWorkflow.match(/pnpm install --frozen-lockfile/g)).toHaveLength(
      ciCacheBlocks.length,
    );
  ...
  it("pins third-party actions in all workflows to commit SHAs", () => {
    ...
    expect(extractTaskBlock(miseToml, "lint:workflow-pins")).toContain(
      "node scripts/check-workflow-pins.mjs",
    );
    expect(readText("scripts/check-workflow-pins.mjs")).toContain(
      '?? ".github/workflows"',
    );
  });
  ...
  it("keeps workflow pin checker rejecting floating quoted uses", () => {
    ...
    try {
      ...
      expect(() => runWorkflowPinChecker(tempDir)).toThrow(
        /actions\/checkout@v6 must use a 40-character commit SHA/,
      );
    } finally {
      rmSync(tempDir, { force: true, recursive: true });
    }
  ...
  it("keeps release workflow permissions limited to release asset publishing", () => {
    const releasePermissions = extractYamlScalarBlockEntries(
      extractTopLevelYamlBlock(releaseWorkflow, "permissions"),
    );
  ...
  it("keeps release workflow action, token, and cache surfaces pinned to the release asset scope", () => {
    ...
    expect(
      extractReleaseStepBlock(
        releaseWorkflow,
        "Validate release signing preflight",
      ),
    ).toContain(
      "TAURI_SIGNING_PRIVATE_KEY_SET: $" +
    ...
    expect(
      ...
      "TAURI_SIGNING_PRIVATE_KEY_PASSWORD_SET: $" +
    ...
    ).toContain("GH_TOKEN: $" + "{{ secrets.GITHUB_TOKEN }}");
    expect(
      extractReleaseStepBlock(
        releaseWorkflow,
    ...
    expect(extractReleaseCacheBlock(releaseWorkflow)).not.toContain(
      "node_modules",
    );
  });
  ...
  it("keeps CI apt mirror failures bounded by an explicit retry policy", () => {
    expect(
      ciWorkflow.match(/sudo apt-get update -o Acquire::Retries=3/g),
    ).toHaveLength(2);
    expect(
      ciWorkflow.match(
  ...
  it("keeps actionlint shellcheck disabled only with a paired shell gate", () => {
    ...
    expect(extractTaskBlock(miseToml, "lint:actions")).toContain(
      "actionlint -shellcheck=",
    );
    expect(extractTaskBlock(miseToml, "lint:actions-shell")).toContain(
      'run = "actionlint"',
    );
  });
  ...
  it("documents the intentionally narrow Windows Rust test scope", () => {
    ...
    expect(extractTaskBlock(miseToml, "test:ci")).toContain(
      'depends = ["test:rust", "test:unit:ci"]',
    );
  ...
  it("keeps release artifact display metadata source-of-truth explicit", () => {
    ...
    expect(extractTomlString(cargoToml, "description")).toBe(
      "A Tauri-based RSS reader",
    );
  ...
  it("requires the release workflow to build with the release updater config", () => {
    ...
    expect(releaseWorkflow).toContain(
      `const releaseConfigPath = "${RELEASE_TAURI_CONFIG_PATH}";`,
    );
    expect(releaseWorkflow).toContain(
      `const devConfigPath = "${DEV_TAURI_CONFIG_PATH}";`,
    );
    expect(releaseWorkflow).toContain(
      "src-tauri/tauri.release.conf.json must enable updater artifacts",
    );
    expect(releaseWorkflow).toContain(
  ...
  it("keeps Tauri CSP explicit across release and dev HMR boundaries", () => {
    ...
    for (const [directive, requiredSources] of Object.entries(
      REQUIRED_RELEASE_CSP_DIRECTIVES,
    )) {
      expect(directives.get(directive)).toEqual(
        expect.arrayContaining([...requiredSources]),
      );
    }
    for (const [directive, sources] of directives) {
      for (const forbiddenSource of RELEASE_CSP_FORBIDDEN_SOURCES) {
        expect(
          sources,
          `${directive} must not include ${forbiddenSource}`,
        ).not.toContain(forbiddenSource);
      }
    }
  ...
  it("keeps Windows manifest, build script, and generated capability schema in the local release gate", () => {
    ...
    const desktopSchema = JSON.parse(
      readText("src-tauri/gen/schemas/desktop-schema.json"),
    ) as unknown;
    const windowsSchema = JSON.parse(
      readText("src-tauri/gen/schemas/windows-schema.json"),
    ) as unknown;
    const macosSchema = JSON.parse(
      readText("src-tauri/gen/schemas/macOS-schema.json"),
    ) as unknown;
    const aclManifests = JSON.parse(
      readText("src-tauri/gen/schemas/acl-manifests.json"),
    ...
    expect(buildScript).toContain(
      "Unsupported target arch for WebView2 loader copy",
    );
    expect(buildScript).toContain("Failed to copy WebView2Loader.dll to");
    expect(buildScript).not.toContain(
      "cargo:warning=Could not find WebView2Loader.dll to copy",
    );
    expect(buildScript).not.toContain(
      "cargo:warning=Failed to copy WebView2Loader.dll",
    );
    ...
    expect(defaultCapabilitySource).toContain(
      '"$schema": "../gen/schemas/desktop-schema.json"',
    );
    expect(desktopSchema).toEqual(windowsSchema);
    expect(desktopSchema).toEqual(macosSchema);
    expect(Object.keys(aclManifests)).toEqual(
      expect.arrayContaining(["core", "opener", "updater"]),
    );
  });
  ...
  it("keeps bundle identifier, release updater artifacts, and updater endpoint in one release contract", () => {
    ...
    expect(tauriConfig.plugins?.updater?.endpoints).toEqual([
      RELEASE_UPDATER_ENDPOINT,
    ]);
    expect(tauriConfig.plugins?.updater?.pubkey).toBeTruthy();
    expect(tauriConfig.plugins?.updater?.pubkey).not.toMatch(
      UPDATER_PUBKEY_PLACEHOLDER_PATTERN,
    );
    expect(releaseWorkflow).toContain(RELEASE_UPDATER_ENDPOINT);
    expect(releaseWorkflow).toContain(
      "src-tauri/tauri.conf.json updater pubkey must be configured",
    );
  });
  ...
  it("documents the production app data namespace migration path before any bundle identifier change", () => {
    ...
    expect(incidentRunbook).toContain(
      "The current production bundle identifier is `com.jey3dayo.ultra-rss-reader`",
    );
    expect(incidentRunbook).toContain(
      "triage must check the old identifier's app data, log, and keyring namespace",
    );
    expect(incidentRunbook).toContain(
      "OS keyring credentials may need to be re-entered",
    );
    expect(incidentRunbook).toContain(
      "Do not move logs, backups, support dumps, or credentials between identifier namespaces",
    );
    expect(incidentRunbook).toContain(
      "normal startup must not rename the app data directory automatically",
    );
    expect(incidentRunbook).toContain(
      "Log paths change with the identifier namespace",
    );
    expect(incidentRunbook).toContain(
      "Rollback across an identifier change must use the old identifier namespace",
    );
    expect(releaseManualVerification).toContain(
      "No automatic app data directory rename is attempted during normal startup",
    );
    expect(releaseManualVerification).toContain(
      "a user-visible database migration prompt or backup/copy prompt",
    );
    expect(releaseManualVerification).toContain(
      "OS keyring credentials cannot be copied automatically",
    );
    expect(releaseManualVerification).toContain(
  ...
  it("keeps privacy-sensitive export, reset, support dump, and settings portability contracts documented", () => {
    ...
    expect(incidentRunbook).toContain(
      "Treat database backup sets as private, unencrypted user data.",
    );
    expect(releaseManualVerification).toContain(
  ...
  it("keeps reader import, favicon, and browser-origin privacy boundaries documented", () => {
    ...
    expect(feedContentPrivacy).toContain(
      "Favicon requests must send no `Referer` header.",
    );
    expect(feedContentPrivacy).toContain("use a maximum 7-day success TTL");
    expect(feedContentPrivacy).toContain(
      "Failure cache must be bounded, resettable, and expire within 24 hours.",
    );
    expect(feedContentPrivacy).toContain(
      "The embedded browser webview represents a remote publisher origin.",
    );
    expect(feedContentPrivacy).toContain(
    ...
    expect(feedContentPrivacy).toContain(
      "Ultra RSS Reader does not currently integrate with OS reduced-data or",
    );
    expect(feedContentPrivacy).toContain(
      "Reader remote images continue to load in the default mode",
    );
    expect(feedContentPrivacy).toContain(
      "A future reduced-data setting may block reader remote images",
    );
    expect(feedContentPrivacy).toContain(
      "Feed favicon fetching is non-essential remote metadata.",
    );
    expect(feedContentPrivacy).toContain(
  ...
  it("keeps feed provider abuse-prevention and redirect contracts documented", () => {
    expect(providerHttpDefaultsSource).toContain(
    ...
    expect(localProviderSource).toContain(
    ...
    expect(localProviderSource).toContain(
      "local_provider_limits_concurrent_feed_requests_per_instance",
    );
    expect(localProviderSource).toContain(
      "local_provider_keeps_sync_and_discovery_request_limits_separate",
    );
    expect(localProviderSource).toContain(
      "redirect_policy_preserves_authorization_on_same_origin_redirects",
    );
    expect(localProviderSource).toContain(
      "redirect_policy_strips_authorization_on_cross_origin_redirects",
    );
    expect(localProviderSource).toContain(
      "validate_external_feed_url_rechecks_repeated_private_host_resolution",
    );
    expect(localProviderSource).toContain(
      "pull_entries_preserves_retry_after_for_rate_limit_status",
    );
    expect(localProviderSource).toContain(
      "create_subscription_preserves_retry_after_for_rate_limit_status",
    );

    expect(feedDiscoverySource).toContain(
      "Discovery is a user-initiated single URL probe, not a crawler.",
    );
    expect(feedDiscoverySource).toContain(
      "validate_discovery_redirect_rejects_dns_rebinding_private_hostname_targets",
    );
    expect(feedDiscoverySource).toContain(
      "discovery_http_client_sends_shared_user_agent_and_does_not_prefetch_robots",
    );

    expect(feedContentPrivacy).toContain(
      "Local provider sync is single-flight per provider instance.",
    );
    expect(feedContentPrivacy).toContain(
      "The global local sync cap is 1, which also caps same-host sync concurrency at 1",
    );
    expect(feedContentPrivacy).toContain(
      "Manual sync is allowed to bypass automatic-scheduler suppression",
    );
    expect(feedContentPrivacy).toContain(
      "Feed discovery is a user-initiated single URL probe, not a crawler.",
    );
    expect(feedContentPrivacy).toContain(
      "Authorization headers may remain on same-origin redirects",
    );
    expect(feedContentPrivacy).toContain(
      "Authorization headers must be stripped by the HTTP client on cross-origin redirects.",
    );
    expect(feedContentPrivacy).toContain(
      "Redirect targets are revalidated and re-resolved at every redirect hop",
    );
    expect(feedContentPrivacy).toContain(
      "DNS results are not cached by the app-level policy today.",
    );
    expect(incidentRunbook).toContain(
      "Manual sync can bypass automatic-scheduler suppression",
    );
    expect(incidentRunbook).toContain(
      "Feed discovery is a user-initiated single URL probe, not a crawler",
    );
  ...
  it("keeps production release log timezone policy synchronized with support docs", () => {
    ...
    expect(tauriLib).toContain(
      'RELEASE_LOG_TIMEZONE_STRATEGY: &str = "UseLocal"',
    );
    ...
    expect(releaseManualVerification).toContain(
      "Do not convert local release log timestamps to UTC",
    );
    expect(incidentRunbook).toContain(
      "instead of converting timestamps in place",
    );
  });
  ...
  it("keeps packaged window icon paths and runtime platform fallback in the release smoke contract", () => {
    ...
    expect(
      PACKAGED_WINDOW_ICON_PATHS.some((iconPath) => iconPath.endsWith(".icns")),
    ).toBe(true);
    expect(
      PACKAGED_WINDOW_ICON_PATHS.some((iconPath) => iconPath.endsWith(".ico")),
    ).toBe(true);
    expect(
      PACKAGED_WINDOW_ICON_PATHS.some((iconPath) => iconPath.endsWith(".png")),
    ).toBe(true);
    ...
    expect(appIconThemeSource).toContain(
      'logRuntimeDiagnostic("app-icon-theme"',
    );
  });
  ...
  it("keeps generated Android and iOS app icon assets pinned to the release smoke contract", () => {
    ...
    const generatedIconPaths = [
      ...readdirSync("src-tauri/icons/android", { recursive: true }).map(
        (entry) => `android/${entry}`,
      ),
      ...readdirSync("src-tauri/icons/ios", { recursive: true }).map(
        (entry) => `ios/${entry}`,
      ),
    ]
      .filter((entry): entry is string => typeof entry === "string")
    ...
    for (const [iconPath, expectedHash] of Object.entries(
      MOBILE_ICON_ASSET_HASHES,
    )) {
    ...
    expect(readText("docs/README.md")).toContain(
      "`src-tauri/icons/icon.png` is the checked-in source image",
    );
  });
  ...
  it("keeps updater manifest platforms mapped back to release assets and checksums", () => {
    ...
    for (const contract of RELEASE_UPDATER_ASSET_CONTRACT) {
      ...
      expect(releaseWorkflow).toContain(
        `artifactPlatform: "${contract.artifactPlatform}"`,
      );
      expect(releaseWorkflow).toContain(
        `artifactArch: "${contract.artifactArch}"`,
      );
      expect(releaseWorkflow).toContain(
        `matrixPlatform: "${contract.matrixPlatform}"`,
      );
      expect(releaseWorkflow).toContain(
        `matrixArgs: ${JSON.stringify(contract.matrixArgs)}`,
      );
      expect(releaseWorkflow).toContain(
        `assetPattern: "${contract.assetPattern}"`,
      );
      expect(releaseWorkflow).toContain(
        `signaturePattern: "${contract.signaturePattern}"`,
      );
      expect(releaseWorkflow).toContain(
        `checksumPattern: "${contract.checksumPattern}"`,
      );
      ...
      expect(releaseWorkflow).toContain(
        `updater_asset_pattern: ${contract.assetPattern}`,
      );
      expect(releaseWorkflow).toContain(
        `updater_signature_pattern: ${contract.signaturePattern}`,
      );
    ...
    for (const unsupportedPlatformKey of UNSUPPORTED_UPDATER_PLATFORM_KEYS) {
      expect(releaseWorkflow).toContain(
        `unsupportedUpdaterPlatformKeys = ["linux-x86_64", "linux-aarch64"]`,
      );
      expect(releaseWorkflow).not.toContain(
        `platformKey: "${unsupportedPlatformKey}"`,
      );
    }
  });
  ...
  it("keeps release artifact provenance evidence tied to tag, workflow, checksum, and SBOM records", () => {
    ...
    expect(releaseWorkflow).toContain(
      "pnpm-licenses-$" + "{assetPlatform}.json",
    );
    expect(releaseWorkflow).toContain(
      "cargo-licenses-$" + "{assetPlatform}.json",
    );
    expect(releaseWorkflow).toContain(
      "release-provenance-$" + "{assetPlatform}.json",
    );
    expect(releaseWorkflow).toContain("workflowRunUrl");
    expect(releaseWorkflow).toContain("tagTargetSha");
    expect(releaseWorkflow).toContain(
      'execFileSync("git", ["log", "-1", "--format=%s", sourceSha]',
    );
    ...
    expect(releaseWorkflow).toContain(
      'execFileSync("git", ["rev-parse", `refs/tags/$' +
        "{process.env.RELEASE_TAG}^{}`]",
    );
    expect(releaseWorkflow).toContain("checksumAssetName");
    expect(releaseWorkflow).toContain(
      "expected three release provenance assets",
    );
    expect(releaseWorkflow).toContain(
      "release provenance source $" +
        "{sourceSha} does not match tag target $" +
        "{tagTargetSha}",
    );
    expect(releaseWorkflow).toContain(
      "releaseDraft: $" + "{{ steps.release-policy.outputs.draft }}",
    ...
    expect(
      releaseWorkflow.indexOf("Generate updater asset checksums"),
    ).toBeLessThan(
      releaseWorkflow.indexOf("Generate release provenance record"),
    );
    expect(
      releaseWorkflow.indexOf("Generate release provenance record"),
    ).toBeLessThan(releaseWorkflow.indexOf("Upload release provenance assets"));
    expect(releaseManualVerification).toContain(
      "Release Provenance And SBOM Record",
    );
    ...
    expect(releaseManualVerification).toContain(
      "Quarantine and first-launch result",
    );
    expect(releaseManualVerification).toContain(
      "Update check smoke result from the installed published artifact",
    );
    expect(releaseManualVerification).toContain(
      "Windows Installer Signing And SmartScreen Verification",
    );
    expect(releaseManualVerification).toContain(
  ...
  it("stops release artifact publication when signing secrets are missing and keeps dry-run preflight explicit", () => {
    const signingPreflightStep = extractReleaseStepBlock(
      releaseWorkflow,
      "Validate release signing preflight",
    );
    ...
    expect(signingPreflightStep).toContain(
      "release dry run validated source, versions, cache, and signing preflight",
    );
    expect(signingPreflightStep).toContain(
    ...
    expect(signingPreflightStep).toContain(
      "rerun workflow_dispatch with dry_run=true",
    );
    expect(signingPreflightStep).toContain(
    ...
    expect(
      releaseWorkflow.indexOf("Validate release signing preflight"),
    ).toBeLessThan(releaseWorkflow.indexOf("Preflight release build"));
    expect(
      releaseWorkflow.indexOf("Validate release signing preflight"),
    ).toBeLessThan(releaseWorkflow.indexOf("tauri-apps/tauri-action"));
    expect(releaseManualVerification).toContain(
      "workflow_dispatch` used `dry_run=true`",
    );
  });

  it("keeps Release Drafter config separate from release workflow publishing responsibilities", () => {
    expect(releaseConfig).toContain(
      "Release Drafter only owns PR-label changelog grouping",
    );
    expect(releaseConfig).toContain(
      ".github/workflows/release.yml owns tag validation",
    );
  ...
  it("keeps release note publication owned by the release skill with prerelease and build metadata policy", () => {
    ...
    expect(releaseSkill).toContain(
      "Write release notes and `CHANGELOG.md` entries in concise Japanese",
    );
    ...
    expect(releaseSkill).toContain(
      "Treat the CLI as the source of truth for release note body text",
    );
    expect(releaseSkill).toContain(
      "After create/edit, verify with `gh release view",
    );
    expect(releaseSkill).toContain(
  ...
  it("keeps release notes and updater messages classified from the same user-visible change set", () => {
    ...
    expect(releaseManualVerification).toContain(
      "Data migration or storage compatibility change",
    );
    expect(releaseManualVerification).toContain("Manual action required");
    expect(releaseManualVerification).toContain(
      "Rollback impossible or unsafe",
    );
  ...
  it("keeps public known-issue copy separate from internal TODO risk tracking", () => {
    ...
    expect(releaseManualVerification).toContain(
      "Internal-only risk may stay in `TODO.md`",
    );
    expect(releaseManualVerification).toContain(
    ...
    expect(releaseManualVerification).toContain(
      "A known issue should include a workaround when one exists",
    );
  });

  it("keeps published macOS artifact notarization, quarantine, and translocation manual checks explicit", () => {
    expect(releaseManualVerification).toContain(
      "published macOS artifact downloaded through the normal browser",
    );
    expect(releaseManualVerification).toContain(
      "not a locally rebuilt or re-signed app",
    );
    expect(releaseManualVerification).toContain("com.apple.quarantine");
    expect(releaseManualVerification).toContain(
      "Gatekeeper and notarization result before first launch",
    );
    expect(releaseManualVerification).toContain(
      "does not require removing quarantine manually",
    );
    expect(releaseManualVerification).toContain("translocation evidence");
    expect(releaseManualVerification).toContain(
      "Do not work around it by clearing quarantine on the verifier machine",
    );
  });
  ...
  it("keeps release hotfix scope and evidence separate from the normal release checklist", () => {
    ...
    expect(releaseManualVerification).toContain(
      "affected version, regression, user impact, and rollback option",
    );
    expect(releaseManualVerification).toContain(
      "contains only the fix, required tests, and release notes for that regression",
    );
    expect(releaseManualVerification).toContain(
      "record the skipped gate and the reason",
    );
    expect(releaseManualVerification).toContain(
      "old artifact digest, replacement artifact digest",
    );
    expect(releaseManualVerification).toContain(
  ...
  it("keeps release builds from using dev Tauri config or dev credentials", () => {
    const tauriActionBlock = extractTauriActionBlock(releaseWorkflow);
    const devOnlyImportPattern =
      /(?:from\s+|import\()\s*["']@\/dev\/(?:mock-data|scenarios)(?:\/|["'])/;
    const releaseSourceDevOnlyImports = listTypeScriptSourceFiles(
      "src",
    ).flatMap((filePath) => {
      if (
    ...
    expect(releaseWorkflow).toContain(
      "release build must keep dev browser mocks disabled inside Tauri",
    );
    expect(releaseWorkflow).toContain(
      "release source must not import dev-only mock data or scenario modules",
    );
    expect(packageJson.scripts).toMatchObject({
      "check:release-contamination":
        "node ./scripts/check-release-build-contamination.ts",
    });
    ...
    expect(tauriActionBlock).not.toContain(
  ...
  it("keeps release manual checks covering first-run prompts, permission denials, and Windows crash visibility", () => {
    ...
    expect(releaseManualVerification).toContain(
      "First-Run Permission Prompt Verification",
    );
    expect(releaseManualVerification).toContain(
      "first-run prompts appear only after user-initiated actions",
    );
    ...
    expect(releaseManualVerification).toContain(
      "Windows Hidden Console And Crash Visibility Verification",
    );
    expect(releaseManualVerification).toContain(
      "Normal launch does not leave an unexpected console window",
    );
    expect(releaseManualVerification).toContain(
      "release logs without requiring a visible console",
    );
  ...
  it("keeps updater, export, and database backup interruption checks cancellation-aware", () => {
    ...
    expect(incidentRunbook).toContain(
      "Canceling an updater download must leave no installable pending artifact.",
    );
    expect(incidentRunbook).toContain(
  ...
  it("keeps single-instance and deep-link routing behind lifecycle state gates", () => {
    ...
    expect(feedContentPrivacy).toContain(
      "reserved routes for `focus`, `settings`, and `import-preview`",
    );
    expect(feedContentPrivacy).toContain(
  ...
  it("keeps Tauri identifiers and seed data directories collision-proof across dev and production", () => {
    ...
    expect(seedDevDatabaseScript).toContain(
      `const PROD_APP_IDENTIFIER = "${PROD_TAURI_IDENTIFIER}";`,
    );
    expect(seedDevDatabaseScript).toContain(
      `const DEV_APP_IDENTIFIER = "${DEV_TAURI_IDENTIFIER}";`,
    );
    expect(seedDevDatabaseScript).toContain(
      'readConfiguredEnvValue(env, "ULTRA_RSS_PROD_APP_DATA_DIR")',
    );
    expect(seedDevDatabaseScript).toContain(
      'readConfiguredEnvValue(env, "ULTRA_RSS_DEV_APP_DATA_DIR")',
    );
    expect(seedDevDatabaseScript).toContain(
      "Production and Dev app data directories resolve to the same path.",
    );
    expect(seedDevDatabaseScript).toContain(
      "Refusing to seed from a Dev app data directory.",
    );
    expect(seedDevDatabaseScript).toContain(
  ...
  it("requires provider account kind additions to update capability and add-account contracts", () => {
    const providerKinds = extractRustEnumVariants(
      providerSource,
      "ProviderKind",
    );
    const enabledServiceKinds = extractEnabledServiceKinds(
      addAccountServicesSource,
    );

    expect(providerKinds).toEqual(["Local", "FreshRss", "Quarantined"]);
    expect(enabledServiceKinds).toEqual(["Local", "FreshRss"]);
    expect(providerSource).toContain(
      "provider_capability_matrix_is_fixed_by_account_kind",
    );
    ...
    expect(providerSource).toContain(
      "ProviderSideDeletionRetention::RetainLocal",
    );
    expect(feedContentPrivacy).toContain(
    ...
    expect(addAccountFormSource).toContain(
    ...
    expect(addAccountServicesSource).toContain(
  ...
  it("keeps stale update installs gated by updater policy and database schema compatibility", () => {
    ...
    expect(migrationSource).toContain(
      "Downgrade startup is blocked to avoid data loss",
    );
    expect(migrationSource).toContain(
      "fn future_schema_version_blocks_downgrade_migration()",
    );
    expect(incidentRunbook).toContain(
      "Stale update install must be rejected or surfaced as recovery-required",
    );
    expect(incidentRunbook).toContain(
      "Starting an older app against a newer database schema is a downgrade attempt",
    );
    expect(releaseManualVerification).toContain(
      "Rollback impossible or unsafe",
    );
  ...
  it("keeps native menu action payloads aligned with frontend AppAction ids", () => {
    const nativeMenuActions =
      extractNativeMenuActionContracts(nativeMenuSource);
    ...
    for (const [menuId, action] of nativeMenuActions) {
      expect(
        appActions.has(action),
        `${menuId} emits ${action}, but APP_ACTIONS does not accept it`,
      ).toBe(true);
    }
    ...
    expect(appActionsSource).toContain(
      'export type AppActionSurface = "commandPalette" | "nativeMenu"',
    );
  ...
  it("keeps native menu item action ids aligned with customizable shortcut definitions", () => {
    ...
    for (const [
      ...
      action,
      shortcutActionId,
    ] of menuActionShortcutContracts) {
  ...
  it("keeps browser webview capability on a minimal command surface", () => {
    ...
    const browserCapability = capabilityByIdentifier(
      defaultCapability,
      "browser-webview",
    );
    const browserPermissionIds =
      browserCapability.permissions?.map(permissionIdentifier) ?? [];
    ...
    expect(
      browserPermissionIds.some((permission) =>
  ...
  it("keeps external opener capability scope aligned with the frontend URL schema", () => {
    ...
    const openerPermission = mainCapability.permissions?.find(
      (permission) =>
        permissionIdentifier(permission) === "opener:allow-open-url",
    );

    expect(mainCapability.permissions?.map(permissionIdentifier)).not.toContain(
  ...
  it("keeps native checked menu preferences compatible with frontend preference migration", () => {
    ...
    expect(nativeMenuSource).toContain(
      "should_rollback_check_toggle_after_emit(toggled_check_item, true)",
    );
    ...
    expect(preferencesSchemaSource).toContain(
  ...
  it("generates a release/debug feature flag inventory report", () => {
    ...
    expect(packageJson.scripts).toMatchObject({
      "report:release-debug-flags":
        "node ./scripts/release-debug-feature-flags-report.ts",
    });
    expect(report.generatedBy).toBe(
      "scripts/release-debug-feature-flags-report.ts",
    );
  ...
  it("keeps markdownlint target count and ignore patterns under a repo contract", () => {
    const markdownlintConfig = parseJsonc(
      readText(".markdownlint-cli2.jsonc"),
    ) as {
      globs?: string[];
      ignores?: string[];
    };
    ...
    expect(markdownlintConfig.ignores).toEqual([
      ...markdownlintRepoContract.ignorePatterns,
    ]);
    expect(markdownFiles).toHaveLength(
      markdownlintRepoContract.targetFileCount,
    );
    expect(
      markdownlintRepoContract.rootMarkdownFiles.every((path) =>
        markdownFiles.includes(path),
      ),
    ).toBe(true);
    expect(
      markdownFiles.some((path) => path.startsWith("src-tauri/gen/")),
    ).toBe(false);
    expect(miseToml).toContain('[tasks."quality:markdownlint-contract"]');
    expect(miseToml).toContain(
      `expected ${markdownlintRepoContract.targetFileCount}`,
    );
  ...
  it("keeps generated fixture, snapshot, and report artifact size budgets under a repo contract", () => {
    ...
    const fixtureFiles = repoFiles.filter((path) =>
      generatedFixtureSnapshotSizeBudget.fixturePathPrefixes.some((prefix) =>
        path.startsWith(prefix),
      ),
    );
    const snapshotFiles = repoFiles.filter((path) =>
      /(?:^|\/)__snapshots__\/|\.snap(?:\.|$)/.test(path),
    );
    const oversizedFixtureFiles = fixtureFiles.filter(
      (path) =>
        statSync(path).size >
        generatedFixtureSnapshotSizeBudget.maxCheckedInFixtureBytes,
    );

    expect(fixtureFiles.length).toBeGreaterThan(0);
    expect(oversizedFixtureFiles).toEqual([]);
    expect(snapshotFiles).toHaveLength(
      generatedFixtureSnapshotSizeBudget.maxSnapshotFileCount,
    );
    expect(
      generatedFixtureSnapshotSizeBudget.generatedReportIgnoredPathPrefixes.every(
        (prefix) => qualityBaselineRepoScanIgnoredPathPrefixes.includes(prefix),
      ),
    ).toBe(true);
    expect(
      isGeneratedReportArtifactPath("tmp/release-debug-feature-flags.json"),
    ).toBe(true);
    expect(
      isGeneratedReportArtifactPath("tests/fixtures/opml/generated-basic.opml"),
    ).toBe(false);
    expect(
      generatedFixtureSnapshotSizeBudget.largeCorpusDirectoryPrefixes,
    ).toEqual(["tests/fixtures/"]);
    expect(generatedFixtureSnapshotSizeBudget.reviewExceptionPolicy).toContain(
      "repo-contract update",
    );
  });
  ...
  it("documents schema, test fixture, dependency update, and reproducibility gates", () => {
    ...
    expect(docsReadme).toContain(
      "must include a schema or query-key version segment",
    );
    expect(docsReadme).toContain(
      "Generated schema drift becomes a failing gate",
    );
    expect(docsReadme).toContain(
      "Date fixtures must use a frozen clock plus relative offsets",
    );
    expect(docsReadme).toContain("Reproducibility audit policy");
    expect(docsReadme).toContain("must not depend on local app state");
    expect(docsReadme).toContain(
      "Runtime dependencies affect shipped code or native behavior",
    );
    expect(docsReadme).toContain(
      "Build-only dependencies affect compilation, bundling, packaging, or generated assets",
    );
    expect(docsReadme).toContain(
      "Dev-only dependencies affect lint, format, reports, or local-only tooling",
    );
    expect(docsReadme).toContain(
      "Transitive-risk updates are indirect dependency changes",
    );
  });
  ...
  it("keeps release note category labels covered by issue and PR label contracts", () => {
    const issueTemplateLabels = issueTemplateFileNames.flatMap((fileName) =>
      extractYamlInlineListValues(
        readText(`.github/ISSUE_TEMPLATE/${fileName}`),
        "labels",
      ),
    );
    const contractLabels = new Set([
      ...extractYamlTopLevelKeys(labelerConfig),
      ...issueTemplateLabels,
    ]);

    for (const label of extractYamlLabelsFields(releaseConfig)) {
      expect(
        contractLabels.has(label),
        `${label} is not covered by issue templates or .github/labeler.yml`,
      ).toBe(true);
    }
  });
  ...
  it("keeps local labeler and PR insights labeler source-of-truth split explicit", () => {
    ...
    expect(labelerConfig).toContain(
      "this file owns area and release-category labels",
    );
    expect(labelerWorkflow).toContain(
      ".github/labeler.yml owns area and release-category labels",
    );
    expect(prInsightsLabelerWorkflow).toContain(
      "PR Insights owns risk/* and size/* labels only",
    );
    expect(
      localLabelerLabels.filter((label) =>
        prInsightsOwnedPrefixes.some((prefix) => label.startsWith(prefix)),
      ),
    ).toEqual([]);
    expect(
      releaseLabels.filter((label) =>
        prInsightsOwnedPrefixes.some((prefix) => label.startsWith(prefix)),
      ),
    ).toEqual([]);
  });
  ...
  it("keeps issue Done When placeholders tied back to the PR DoD checklist", () => {
    ...
    for (const fileName of issueTemplateFileNames) {
      ...
      const doneWhenDescription =
        extractIssueTemplateDoneWhenDescription(source);
      const doneWhenPlaceholder =
        extractIssueTemplateDoneWhenPlaceholder(source);

      expect(
        doneWhenDescription,
        `${fileName} Done When should classify gate differences`,
      ).toContain("PR DoD 共通 gate");
      expect(
        doneWhenDescription,
        `${fileName} Done When should classify gate differences`,
      ).toContain("固有 gate");
      expect(
        doneWhenDescription,
        `${fileName} Done When should classify gate differences`,
      ).toContain("manual verification gate");
      expect(
        doneWhenPlaceholder,
        `${fileName} Done When should reference PR DoD`,
      ).toContain("PR 作成時は PR template の確認済み DoD を満たす");
    }
  });
});
